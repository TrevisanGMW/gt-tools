...
# Auto Scalable Create Stretchy Legs
...
def changeOutlinerColor(objList, colorListRGB):
    for obj in objList:
        cmds.setAttr ( obj + ".useOutlinerColor" , True)
        cmds.setAttr ( obj + ".outlinerColor" , colorListRGB[0],colorListRGB[1],colorListRGB[2])
...
distanceNodeOne = cmds.rename(distanceOneTransform, nameDistanceNode + "_Strechy_Term_01")
...
distanceNodeTwo = cmds.rename(distanceTwoTransform, nameDistanceNode + "_Strechy_Condition_01")
...
stretchNormalizationNode = cmds.createNode('multiplyDivide')
