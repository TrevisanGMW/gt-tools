    ...
    for obj in selection:
        ...
        if gt_sel_manager.get('use_contains_type'):
            for string in gt_sel_manager.get('stored_contains_type'):
                ...
                if gt_sel_manager.get('stored_shape_node_type') == "Select Both Parent and Shape" and string in obj_shape_type or string in obj_type:
                    to_add_list.append(obj)
        ...
        if gt_sel_manager.get('use_visibility_state') == True or gt_sel_manager.get('use_outliner_color') == True or gt_sel_manager.get('use_no_outliner_color') == True:
            obj_attr_list = cmds.listAttr(obj) or []
        
        # Check Visibility State
        if gt_sel_manager.get('use_visibility_state') == True and gt_sel_manager.get('stored_visibility_state') == True:
            if len(obj_attr_list) > 0 and "visibility" in obj_attr_list:
                if cmds.getAttr(obj + ".visibility"):
                    to_add_list.append(obj)
        
        if gt_sel_manager.get('use_visibility_state') == True and gt_sel_manager.get('stored_visibility_state') == False:
            if len(obj_attr_list) > 0 and "visibility" in obj_attr_list:
                if cmds.getAttr(obj + ".visibility"):
                    to_remove_list.append(obj)
...
def selection_feedback(number_objects):
    ...
    '''
    if number_objects != 0:
            message = '<' + str(random.random()) + '><span style=\"color:#FF0000;text-decoration:underline;\">' + str(number_objects)
