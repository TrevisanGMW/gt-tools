...
import gt.tools.auto_rigger.modules.module_arm as module_arm
...
class TestModuleArm(unittest.TestCase):
    def setUp(self):
    ...
    def test_module_biped_arm_inheritance(self):
        arm_module_instance = module_arm.ModuleArm()
        result = isinstance(arm_module_instance, tools_rig_frm.ModuleGeneric)
        expected = True
        self.assertEqual(expected, result)

    def test_module_arm_inheritance_passthrough(self):
        arm_module_instance = module_arm.ModuleArm(name="mocked_arm", prefix="mocked_prefix", suffix="mocked_suffix")
    ...
    def test_module_arm_get_module_as_dict(self):
        ...
        expected_module_value = "ModuleArm"
        ...
        a_2nd_arm_module = module_arm.ModuleArm()
    ...
    def test_module_arm_prefixes(self):
        a_lf_arm_module = module_arm.ModuleArmLeft()
        a_proxy = a_lf_arm_module.get_prefix()
        expected = "L"
        self.assertEqual(expected, a_proxy)
        a_rt_arm_module = module_arm.ModuleArmRight()
    ...
    def test_module_arm_build_rig(self):
        arm_module_instance = module_arm.ModuleArm(prefix="C")
        a_project = tools_rig_frm.RigProject()
        a_project.add_to_modules(arm_module_instance)
        a_project.build_proxy()
        a_project.build_rig()
        expected_elements = [
            ...
            "C_lowerArmTwist01_JNT",
            "C_lowerArmTwist02_JNT",
            "C_lowerArmTwist03_JNT",
            "C_lowerArm_JNT_parentConstraint",
            "C_upperArmTwist01_JNT",
            "C_upperArmTwist02_JNT",
            "C_upperArmTwist03_JNT",
            "C_upperArm_JNT_parentConstraint",
            "C_clavicle_JNT_parentConstraint",
            "C_clavicle_offset",
            "C_clavicle_CTRL",
            "C_upperArm_offset",
            "C_upperArm_parentOffset",
            "C_upperArm_CTRL",
            "C_lowerArm_offset",
            "C_lowerArm_CTRL",
            "C_hand_offset",
    ...
    def test_module_arm_find_drivers_from_module(self):
        a_arm_module = module_arm.ModuleArm(prefix="C")
        # Setup Project
        a_project = tools_rig_frm.RigProject()
        ...
        expected = [
            "|rig|controls|C_global_CTRL|C_globalOffset_CTRL|C_arm_offset|C_arm_CTRL",
    ...
    def test_module_arm_proxy_pose_default(self):
        an_arm_module = module_arm.ModuleArm()
        # Setup Project
        ...
        world_position_proxy = [
            ...
            ("upperArm", [0.0, 0.0, 17.2]),
            ("lowerArm", [0.0, -0.01, 37.7]),
            ("hand", [0.0, 0.0, 58.2]),
        ]
        ...
        world_rotation_proxy = [
            ("clavicle", [-180.0, -90.0, 0.0]),
            ("upperArm", [0.0, 0.0, -90.0]),
            ("lowerArm", [-90.0, -90.0, 0.0]),
            ("hand", [-180.0, -90.0, 0.0]),
        ]
    ...
    def test_module_arm_joint_pose_default(self):
        an_arm_module = module_arm.ModuleArm(clavicle_world=False)
        # Setup Project
        a_proxy = tools_rig_frm.Proxy()
        a_project = tools_rig_frm.RigProject()
        a_project.add_to_modules(an_arm_module)
        ...
        world_position_joint = [
            ("clavicle_JNT", [0.0, 0.0, 0.0]),
            ("upperArm_JNT", [0.0, -0.0, 17.2]),
            ("lowerArm_JNT", [0.0, -0.01, 37.7]),
            ("hand_JNT", [0.0, -0.0, 58.2]),
            # FK joints
            ("clavicle_JNT_fk", [0.0, 0.0, 0.0]),
            ("upperArm_JNT_fk", [0.0, -0.0, 17.2]),
            ("lowerArm_JNT_fk", [0.0, -0.01, 37.7]),
            ("hand_JNT_fk", [0.0, -0.0, 58.2]),
            # IK joints
            ("clavicle_JNT_ik", [0.0, 0.0, 0.0]),
            ("upperArm_JNT_ik", [0.0, -0.0, 17.2]),
            ("lowerArm_JNT_ik", [0.0, -0.01, 37.7]),
            ("hand_JNT_ik", [0.0, -0.0, 58.2]),
        ]
        ...
        world_rotation_joint = [
            ("clavicle_JNT", [-180.0, -90.0, 0.0]),
            ("upperArm_JNT", [-180.0, -90.0, -0.03]),
            ("lowerArm_JNT", [90.0, -89.97, 90.0]),
            ("hand_JNT", [0.0, 90.0, 180.0]),
            # FK joints
            ("clavicle_JNT_fk", [-180.0, -90.0, 0.0]),
            ("upperArm_JNT_fk", [-180.0, -90.0, -0.03]),
            ("lowerArm_JNT_fk", [90.0, -89.97, 90.0]),
            ("hand_JNT_fk", [0.0, 90.0, 180.0]),
            # IK joints
            ("clavicle_JNT_ik", [-180.0, -90.0, 0.0]),
            ("upperArm_JNT_ik", [-180.0, -90.0, -0.03]),
            ("lowerArm_JNT_ik", [90.0, -89.97, 90.0]),
            ("hand_JNT_ik", [0.0, 90.0, 180.0]),
        ]
    ...
    def test_module_arm_joint_pose_a(self):
        an_arm_module = module_arm.ModuleArm()
        # Setup Project
        an_arm_module.clavicle_proxy.set_initial_position(xyz=[0, 10, 60])
        an_arm_module.upperarm_proxy.set_initial_position(xyz=[10, 10, 60])
        an_arm_module.lowerarm_proxy.set_initial_position(xyz=[10, 10, 0])
        an_arm_module.hand_proxy.set_initial_position(xyz=[40, 10, 60])
        a_proxy = tools_rig_frm.Proxy()
        a_project = tools_rig_frm.RigProject()
        a_project.add_to_modules(an_arm_module)
        a_project.add_to_modules(a_proxy)
        ...
        world_position_joint = [
            ("clavicle_JNT", [0.0, 10.0, 60.0]),
            ("upperArm_JNT", [10.0, 10.0, 60.0]),
            ("lowerArm_JNT", [10.0, 10.0, 0.0]),
            ("hand_JNT", [40.0, 10.0, 60.0]),
            # FK joints
            ("clavicle_JNT_fk", [0.0, 10.0, 60.0]),
            ("upperArm_JNT_fk", [10.0, 10.0, 60.0]),
            ("lowerArm_JNT_fk", [10.0, 10.0, 0.0]),
            ("hand_JNT_fk", [40.0, 10.0, 60.0]),
            # IK joints
            ("clavicle_JNT_ik", [0.0, 10.0, 60.0]),
            ("upperArm_JNT_ik", [10.0, 10.0, 60.0]),
            ("lowerArm_JNT_ik", [10.0, 10.0, 0.0]),
            ("hand_JNT_ik", [40.0, 10.0, 60.0]),
        ]
        ...
        world_rotation_joint = [
            ("clavicle_JNT", [-180.0, -90.0, 0.0]),
            ("upperArm_JNT", [0.0, 90.0, 0.0]),
            ("lowerArm_JNT", [-90.0, -63.43, 0.0]),
            ("hand_JNT", [0.0, 90.0, 180.0]),
            # FK joints
            ("clavicle_JNT_fk", [-180.0, -90.0, 0.0]),
            ("upperArm_JNT_fk", [0.0, 90.0, 0.0]),
            ("lowerArm_JNT_fk", [-90.0, -63.43, 0.0]),
            ("hand_JNT_fk", [0.0, 90.0, 180.0]),
            # IK joints
            ("clavicle_JNT_ik", [-180.0, -90.0, 0.0]),
            ("upperArm_JNT_ik", [0.0, 90.0, 0.0]),
            ("lowerArm_JNT_ik", [-90.0, -63.43, 0.0]),
            ("hand_JNT_ik", [0.0, 90.0, 180.0]),
        ]
    ...
    def test_module_arm_joint_pose_b(self):
        an_arm_module = module_arm.ModuleArm(clavicle_world=False)
        # Setup Project
        an_arm_module.clavicle_proxy.set_initial_position(xyz=[0, 20, 70])
        an_arm_module.upperarm_proxy.set_initial_position(xyz=[10, 10, 80])
        an_arm_module.lowerarm_proxy.set_initial_position(xyz=[20, 20, 0])
        an_arm_module.hand_proxy.set_initial_position(xyz=[40, 10, 80])
        a_project = tools_rig_frm.RigProject()
        a_project.add_to_modules(an_arm_module)
        a_project.build_proxy()
        a_project.build_rig()
        ...
        world_position_joint = [
            ("clavicle_JNT", [0.0, 20.0, 70.0]),
            ("upperArm_JNT", [10, 10, 80]),
            ("lowerArm_JNT", [20, 20, 0]),
            ("hand_JNT", [40, 10, 80]),
            # FK joints
            ("clavicle_JNT_fk", [0.0, 20.0, 70.0]),
            ("upperArm_JNT_fk", [10, 10, 80]),
            ("lowerArm_JNT_fk", [20, 20, 0]),
            ("hand_JNT_fk", [40, 10, 80]),
            # IK joints
            ("clavicle_JNT_ik", [0.0, 20.0, 70.0]),
            ("upperArm_JNT_ik", [10, 10, 80]),
            ("lowerArm_JNT_ik", [20, 20, 0]),
            ("hand_JNT_ik", [40, 10, 80]),
        ]
        ...
        world_rotation_joint = [
            ("clavicle_JNT", [-180.0, -90.0, 0.0]),
            ("upperArm_JNT", [-7.13, 82.87, 7.07]),
            ("lowerArm_JNT", [-62.56, -74.38, -26.57]),
            ("hand_JNT", [0.0, 90.0, 180.0]),
            # FK joints
            ("clavicle_JNT_fk", [-180.0, -90.0, 0.0]),
            ("upperArm_JNT_fk", [-7.13, 82.87, 7.07]),
            ("lowerArm_JNT_fk", [-62.56, -74.38, -26.57]),
            ("hand_JNT_fk", [0.0, 90.0, 180.0]),
            # IK joints
            ("clavicle_JNT_ik", [-180.0, -90.0, 0.0]),
            ("upperArm_JNT_ik", [-7.13, 82.87, 7.07]),
            ("lowerArm_JNT_ik", [-62.56, -74.38, -26.57]),
            ("hand_JNT_ik", [0.0, 90.0, 180.0]),
        ]
    ...
    def test_module_arm_control_pose_default(self):
        ...
        world_positions_controls = [
            ("arm_CTRL", [0.0, -0.0, 58.2]),
            ("clavicle_CTRL", [0.0, 0.0, 0.0]),
            ("upperArm_CTRL", [0.0, -0.0, 17.2]),
            ("lowerArm_CTRL", [0.0, -0.01, 37.7]),
            ("hand_CTRL", [0.0, -0.0, 58.2]),
            ("hand_IK_CTRL", [0.0, -0.0, 58.2]),
            ("lowerArm_IK_CTRL", [0.0, -24.61, 37.7]),
            ("upperArm_JNT_ik", [0.0, 0.0, 17.2]),
        ]
        ...
        world_rotation_controls = [
            ...
            ("clavicle_CTRL", [-180.0, -90.0, 0.0]),
            ("upperArm_CTRL", [-180.0, -90.0, -0.03]),
            ("lowerArm_CTRL", [90.0, -89.97, 90.0]),
            ("hand_CTRL", [0.0, 90.0, 180.0]),
    ...
    def test_module_arm_control_pose_a(self):
        an_arm_module = module_arm.ModuleArm(clavicle_world=False)
        # Setup Project
        an_arm_module.clavicle_proxy.set_initial_position(xyz=[0, 50, 0])
        an_arm_module.upperarm_proxy.set_initial_position(xyz=[10, 40, 0])
        an_arm_module.lowerarm_proxy.set_initial_position(xyz=[10, 20, -10])
        an_arm_module.hand_proxy.set_initial_position(xyz=[10, 0, 0])
        a_proxy = tools_rig_frm.Proxy()
        a_project = tools_rig_frm.RigProject()
        a_project.add_to_modules(an_arm_module)
        a_project.add_to_modules(a_proxy)
        ...
        world_positions_controls = [
            ("arm_CTRL", [10, 0, 0]),
            ("clavicle_CTRL", [0.0, 50, 0.0]),
            ("upperArm_CTRL", [10.0, 40.0, 0]),
            ("lowerArm_CTRL", [10, 20, -10.0]),
            ("hand_CTRL", [10, 0, 0]),
            ("hand_IK_CTRL", [10, 0, 0]),
            ("lowerArm_IK_CTRL", [10, 20, -36.83]),
            ("upperArm_JNT_ik", [10, 40, 0]),
        ]
        ...
        world_rotation_controls = [
            ("arm_CTRL", [0.0, 0.0, 0.0]),
            ("clavicle_CTRL", [-180.0, -90.0, 0.0]),
            ("upperArm_CTRL", [0, 90.0, -63.43]),
            ("lowerArm_CTRL", [-90.0, -26.57, -90.0]),
            ("hand_CTRL", [0.0, 90.0, 180.0]),
            ("hand_IK_CTRL", [0.0, 0.0, 0.0]),
            ("lowerArm_IK_CTRL", [0.0, 0.0, 0.0]),
            ("upperArm_JNT_ik", [0.0, 90.0, -63.43]),
        ]
    ...
    def test_module_arm_control_pose_b(self):
        ...
        world_positions_controls = [
            ("arm_CTRL", [20.0, 52.0, 72.0]),
            ("clavicle_CTRL", [50.0, 110.0, 55.0]),
            ("upperArm_CTRL", [24.0, 54.0, 62.0]),
            ("lowerArm_CTRL", [20.0, 2.0, 1.0]),
            ("hand_CTRL", [20.0, 52.0, 72.0]),
            ("hand_IK_CTRL", [20.0, 52.0, 72.0]),
            ("lowerArm_IK_CTRL", [-10.59, -88.53, -29.34]),
            ("upperArm_JNT_ik", [24.0, 54.0, 62.0]),
        ]
        ...
        world_rotation_controls = [
            ("arm_CTRL", [0.0, 0.0, 0.0]),
            ("clavicle_CTRL", [-180.0, -90.0, 0.0]),
            ("upperArm_CTRL", [3.75, 93.75, -40.39]),
            ("lowerArm_CTRL", [61.58, -54.85, 90.0]),
            ("hand_CTRL", [0.0, 90.0, 180.0]),
            ("hand_IK_CTRL", [0.0, 0.0, 0.0]),
            ("lowerArm_IK_CTRL", [0.0, 0.0, 0.0]),
            ("upperArm_JNT_ik", [3.75, 93.75, -40.39]),
        ]
