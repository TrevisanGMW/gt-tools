...
def build_gui_uv_transfer():
    ...
    window_gui_uv_transfer = cmds.window(
    ...
    cmds.rowColumnLayout(
        nc=3, cw=[(1, 10), (2, 145), (3, 50)], cs=[(1, 10), (2, 0), (3, 0)], p=content_main
    )  # Title Column
    ...
    cmds.rowColumnLayout(
        p=content_main, numberOfColumns=3, columnWidth=[(1, 100), (2, 100), (3, 10)], cs=[(1, 10), (2, 5)]
    )
    ...
    cmds.rowColumnLayout(
        p=content_main, numberOfColumns=1, columnWidth=[(1, 205), (2, 1), (3, 10)], cs=[(1, 10), (2, 5), (3, 5)]
    )
    ...
    icon = ui_qt.QtGui.QIcon(ui_res_lib.Icon.tool_transfer_uvs)
...
def uv_import():
    ...
    try:
        if is_valid:
            file_name = (
                cmds.fileDialog2(
                    fileFilter=script_name + " - Obj File (*.obj)",
                    dialogStyle=2,
                    fileMode=1,
                    okCaption="Import",
                    caption='Importing Obj with UVs for "' + script_name + '"',
                )
                or []
            )
            ...
            if file_exists:
                ...
                if len(imported_shapes) > 0:
                    ...
                    for obj in selection:
                        try:
                            transfer_history = cmds.transferAttributes(
                                imported_objects[0],
                                obj,
                                transferPositions=0,
                                transferNormals=0,
                    ...
                    if transfer_count == 1:
                        ...
                        unique_message += (
                            '</span><span style="color:#FFFFFF;"> ' "object received transferred UVs.</span>"
                        )
                        ...
                        unique_message += str(transfer_count)
                        unique_message += (
                            '</span><span style="color:#FFFFFF;"> ' "objects received transferred UVs.</span>"
                        )
                        cmds.inViewMessage(amg=unique_message, pos="botLeft", fade=True, alpha=0.9)
    ...
    if is_valid:
        file_name = (
            cmds.fileDialog2(
                ...
                caption='Exporting OBJ with UVs for "' + script_name + '"',
            )
            or []
        )
        if len(file_name) > 0:
...
def build_gui_help_uv_transfer():
    ...
    cmds.text(
        l="Script used to quickly transfer UVs between objects.\nIt allows you to export or import UVs or"
    ...
    cmds.text(
        l="Exports an OBJ file containing the model and its UVs\n(similar to simply exporting and OBJ file "
    ...
    cmds.text(
        l="Imports the previously exported OBJ file and transfers\n its UVs to the intermediate object "
    ...
    cmds.text(
        l="Allows you to quickly transfer UVs from the first object\nin your selection to the other objects "
