...
def build_gui_gt_render_checklist():
    ...
    def create_settings_items(items, settings_items, warning_items):
        for item in items:
            ...
            if item in warning_items:
                cmds.textField(
                    "settings_warning_" + str(item), tx=checklist_items.get(item)[1][0], h=14, font=font_size
                )
            ...
            if item in settings_items:
                if item not in warning_items:
                    if isinstance(checklist_items.get(item)[1], list):
                        ...
                        cmds.textField(
                            "settings_1d_error_" + str(item), tx=checklist_items.get(item)[1], h=14, font=font_size
                        )
                        checklist_settings.get("settings_text_fields").append("settings_1d_error_" + str(item))
                else:
                    cmds.textField(
                        "settings_2d_error_" + str(item), tx=checklist_items.get(item)[1][1], h=14, font=font_size
                    )
    ...
    checklist_column = cmds.rowColumnLayout(
        nc=3, cw=[(1, 165), (2, 35), (3, 90)], cs=[(1, 20), (2, 6), (3, 6)], p=main_column
    )
    ...
    save_load_row = cmds.rowColumnLayout(
        nc=2,
        cw=[
            (1, 145),
            (2, 145),
    ...
    cmds.button(
        l="Reset to Default Values", h=30, c=lambda args: settings_apply_changes(reset_default=True), p=settings_buttons
    )
    ...
    def update_gui_settings():
        if not checklist_settings.get("is_settings_visible"):
            ...
            cmds.button(settings_btn, e=True, l="Apply", bgc=(0.6, 0.6, 0.6))
    ...
    icon = ui_qt.QtGui.QIcon(ui_res_lib.Icon.tool_render_checklist)
...
def build_gui_help_gt_render_checklist():
    ...
    cmds.text(
        l="These are also buttons, you can click on them for extra functions:", align="left", fn="smallPlainLabelFont"
    )
    cmds.separator(h=5, style="none")  # Empty Space
    ...
    cmds.button(
        ...
        h=14,
        bgc=def_color,
        c=lambda args: print_message("Default color, means that it was not yet tested.", as_heads_up_message=True),
    )
    ...
    cmds.button(
        ...
        h=14,
        bgc=warning_color,
        c=lambda args: print_message("Warning color, some possible issues were found", as_heads_up_message=True),
    )
    ...
    cmds.button(
        ...
        c=lambda args: print_message(
            "Error color, means that some possible issues were found", as_heads_up_message=True
        ),
    )
    ...
    cmds.text(
        l="This list uses the current settings to show what it expects to find:", align="left", fn="smallPlainLabelFont"
    )
    ...
    message = (
        "[X] "
        + checklist_items.get(0)[0]
        + ': returns error if not matching: "'
    ...
    message = (
        "[X] "
        + checklist_items.get(2)[0]
        + ": returns error if not: "
        + str(checklist_items.get(2)[1])
    ...
    message = (
        "[X] "
        + checklist_items.get(3)[0]
        + ": error if more than "
        + str(checklist_items.get(3)[1][1])
        + "\n     warning if more than "
        + str(checklist_items.get(3)[1][0])
    ...
    message = (
        "[X] "
        + checklist_items.get(4)[0]
        + ": must start with "
        + str(checklist_items.get(4)[1])
    ...
    message = (
        "[X] "
        + checklist_items.get(5)[0]
        + ": must start with "
        + str(checklist_items.get(5)[1])
        + "\n   This function completely ignore slashes.\n   You may use a list as custom value.\n   "
        'Use a comma "," to separate multiple paths\n\n'
    )
    cmds.scrollField(checklist_items_help_scroll_field, e=True, ip=0, it=message)
    ...
    cmds.scrollField(
        checklist_items_help_scroll_field,
        e=True,
        ip=0,
        it="[X] "
        + checklist_items.get(6)[0]
        + ": returns error if common objects are\n     found outside hierarchies"
        + "\n\n",
    )
    ...
    cmds.scrollField(
        ...
        ip=0,
        it="[X] "
        + checklist_items.get(7)[0]
        + ": : error if more than "
        + str(checklist_items.get(7)[1][1])
        + "\n     warning if more than: "
        + str(checklist_items.get(7)[1][0])
        + "."
        + "\n\n",
    )
    ...
    cmds.scrollField(
        ...
        ip=0,
        it="[X] "
        + checklist_items.get(8)[0]
        + ": error if more than "
        + str(checklist_items.get(8)[1][1])
        + "\n     warning if more than "
        + str(checklist_items.get(8)[1][0])
        + "\n\n",
    )
    ...
    cmds.scrollField(
        ...
        ip=0,
        it="[X] "
        + checklist_items.get(9)[0]
        + ": error if more than "
        + str(checklist_items.get(9)[1][1])
        + "\n     warning if more than "
        + str(checklist_items.get(9)[1][0])
        + "."
        + "\n\n",
    )
    ...
    cmds.scrollField(
        ...
        ip=0,
        it="[X] "
        + checklist_items.get(10)[0]
        + ": error if more than "
        + str(checklist_items.get(10)[1][1])
        + "\n     warning if more than "
        + str(checklist_items.get(10)[1][0])
        + "."
        + "\n\n",
    )
    ...
    cmds.scrollField(
        ...
        ip=0,
        it="[X] "
        + checklist_items.get(11)[0]
        + ": error if more than "
        + str(checklist_items.get(11)[1][1])
        + "\n     warning if more than "
        + str(checklist_items.get(11)[1][0])
        + "."
        + "\n\n",
    )
    ...
    message = (
        "[X] "
        + checklist_items.get(12)[0]
        + ": error if using default names."
    ...
    message = (
        "[X] " + checklist_items.get(14)[0] + ": error if any ngons found.\n     "
    ...
    message = (
        "[X] " + checklist_items.get(15)[0] + ": error if is found.\n    A non-manifold geometry is a "
    ...
    message = (
        "[X] " + checklist_items.get(16)[0] + ": error if multiples UV Sets and Empty UV Sets.\n     "
    ...
    message = (
        "[X] "
        + checklist_items.get(17)[0]
        + ": error if rotation(XYZ) not frozen."
    ...
    message = (
        "[X] "
        + checklist_items.get(18)[0]
        + ": error if animated visibility is found"
    ...
    message = (
        "[X] "
        + checklist_items.get(20)[0]
        + ": error if incorrect color space found."
    ...
    message = (
        "[X] "
        + checklist_items.get(21)[0]
        + ": must start with "
        + str(checklist_items.get(21)[1])
        + "\n   This function completely ignore slashes.\n   You may use a list as custom value.\n   "
...
def check_output_resolution():
    ...
    if str(received_value[0]).replace(" ", "") == str(expected_value[0]).replace(" ", "") and str(
        received_value[1]
    ).replace(" ", "") == str(expected_value[1].replace(" ", "")):
        cmds.button(
            "status_" + item_id,
            e=True,
            bgc=pass_color,
    ...
    def patch_output_resolution():
        user_input = cmds.confirmDialog(
            ...
            + item_name.lower()
            + ' from : "'
            + str(received_value[0])
            + "x"
            + str(received_value[1])
            + '" to "'
            + str(expected_value[0])
            + "x"
        ...
        if user_input == "Yes, change it for me":
            try:
                ...
                logger.debug(str(e))
                cmds.warning(
                    'Failed to use custom setting "'
                    + str(expected_value[0])
                    + "x"
                    + str(expected_value[1])
                    + '" as your new resolution.'
                )
            check_output_resolution()
        else:
            cmds.button("status_" + item_id, e=True, l="")
    ...
    if custom_settings_failed:
        string_status = (
            "1 issue found. " "The custom resolution settings provided couldn't be used to check your resolution"
        )
        cmds.button(
            "status_" + item_id,
            e=True,
            ...
            l="",
            c=lambda args: print_message(
                "The custom value provided couldn't be used to check the resolution.", as_warning=True
            ),
        )
    return "\n*** " + item_name + " ***\n" + string_status
...
def check_total_texture_count():
    ...
    if expected_value[1] >= received_value > expected_value[0]:
        ...
        patch_message = (
            "Your " + item_name.lower() + ' is "' + str(received_value) + '" which is a high number.'
...
def check_network_file_paths():
    ...
    if issues_found > 0:
        ...
        for file_node in incorrect_file_nodes:
            string_status = (
                string_status + '"' + file_node + "\" isn't pointing to the the network drive. "
...
def check_network_reference_paths():
    ...
    if len(incorrect_reference_nodes) == 0:
        ...
        cmds.button(
            "status_" + item_id, e=True, bgc=error_color, l="?", c=lambda args: warning_network_reference_paths()
        )
    ...
    def warning_network_reference_paths():
        user_input = cmds.confirmDialog(
            ...
            + "\" of your reference paths aren't pointing to the network drive. "
            "\nPlease change their path to a network location."
            "\n\n(Too see a list of nodes, generate a full report)",
            button=["OK", "Ignore Issue"],
            defaultButton="OK",
...
def check_unparented_objects():
    ...
    if issues_found > 0:
        ...
        for obj in unparented_objects:
            string_status = (
                string_status + '"' + obj + '" has no parent or child nodes. '
...
def check_total_triangle_count():
    ...
    if expected_value > scene_tri_count > inbetween_value:
        ...
        patch_message = (
            "Your scene has "
            + str(scene_tri_count)
            + " triangles. You should try to keep it under "
            + str(expected_value)
            + ".\n\n"
    ...
    if inbetween_value < scene_tri_count < expected_value:
        string_status = (
            ...
            + " issues found. Your scene has "
            + str(scene_tri_count)
            + " triangles, which is high. Consider optimizing it if possible."
        )
    elif scene_tri_count < expected_value:
        string_status = (
            str(issues_found)
            + " issues found. Your scene has "
            + str(scene_tri_count)
            + " triangles. Good job keeping the triangle count low!."
        )
    else:
        string_status = (
            str(issues_found)
            + " issue found. Your scene has "
            + str(scene_tri_count)
            + " triangles. You should try to keep it under "
            + str(expected_value)
            + "."
...
def check_total_poly_object_count():
    ...
    if expected_value > len(all_polymesh) > inbetween_value:
        cmds.button(
            "status_" + item_id, e=True, bgc=warning_color, l="", c=lambda args: warning_total_poly_object_count()
        )
        issues_found = 0
        patch_message = (
            'Your scene contains "'
            + str(len(all_polymesh))
        ...
        cmds.button(
            ...
            bgc=pass_color,
            l="",
            c=lambda args: print_message('Your scene contains "' + str(len(all_polymesh)) + '" polygon meshes.'),
        )
        issues_found = 0
        patch_message = ""
    else:
        cmds.button(
            "status_" + item_id, e=True, bgc=error_color, l="?", c=lambda args: warning_total_poly_object_count()
        )
        issues_found = 1
        patch_message = (
            str(len(all_polymesh))
            + " polygon meshes in your scene. \nTry to keep this number under "
            + str(expected_value)
            + "."
        )
        cancel_message = "Ignore Issue"
    ...
    def warning_total_poly_object_count():
        ...
        if user_input == "Ignore Warning":
            cmds.button(
                ...
                c=lambda args: print_message(
                    str(issues_found)
                    + " issues found. Your scene contains "
                    + str(len(all_polymesh))
                    + " polygon meshes, which is a high number. \nConsider optimizing it if possible."
                ),
            )
    ...
    if expected_value > len(all_polymesh) > inbetween_value:
        string_status = (
            ...
            + ' issues found. Your scene contains "'
            + str(len(all_polymesh))
            + '" polygon meshes, which is a high number. Consider optimizing it if possible.'
        )
    elif len(all_polymesh) < expected_value:
        string_status = (
            str(issues_found) + ' issues found. Your scene contains "' + str(len(all_polymesh)) + '" polygon meshes.'
        )
    else:
        string_status = (
            str(issues_found)
            + ' issue found. Your scene contains "'
            + str(len(all_polymesh))
            + '" polygon meshes. Try to keep this number under "'
            + str(expected_value)
            + '".'
        )
    if custom_settings_failed:
...
def check_shadow_casting_light_count():
    ...
    if expected_value > len(shadow_casting_lights) > inbetween_value:
        cmds.button(
            "status_" + item_id, e=True, bgc=warning_color, l="", c=lambda args: warning_shadow_casting_light_count()
        )
        issues_found = 0
        patch_message = (
            'Your scene contains "'
            + str(len(shadow_casting_lights))
        ...
        cmds.button(
            "status_" + item_id, e=True, bgc=error_color, l="?", c=lambda args: warning_shadow_casting_light_count()
        )
        issues_found = 1
        patch_message = (
            "Your scene contains "
    ...
    if expected_value > len(shadow_casting_lights) > inbetween_value:
        ...
        string_status = (
            ...
            + ' issue found. Your scene contains "'
            + str(len(shadow_casting_lights))
            + '" shadow casting lights, you should keep this number under "'
            + str(expected_value)
            + '".'
        )
    if custom_settings_failed:
        string_status = (
            "1 issue found. " "The custom value provided couldn't be used to check your shadow casting lights."
        )
        cmds.button(
            "status_" + item_id,
            e=True,
...
def check_rs_shadow_casting_light_count():
    ...
    if rs_physical_type in node_types:  # is RS loaded?
        ...
        if expected_value > len(rs_shadow_casting_lights) > inbetween_value:
            ...
            patch_message = (
                'Your scene contains "'
                + str(len(rs_shadow_casting_lights))
        ...
        if expected_value > len(rs_shadow_casting_lights) > inbetween_value:
            ...
            string_status = (
                ...
                + ' issue found. Your scene contains "'
                + str(len(rs_shadow_casting_lights))
                + '" Redshift shadow casting lights, you should '
                'keep this number under "' + str(expected_value) + '".'
            )
        if custom_settings_failed:
            string_status = (
                "1 issue found. The custom value provided couldn't "
        ...
        cmds.button(
            ...
            l="",
            c=lambda args: print_message(
                "No Redshift light types exist in the scene. Redshift plugin doesn't seem to be loaded.",
                as_warning=True,
            ),
        )
...
def check_ai_shadow_casting_light_count():
    ...
    if ai_physical_type in node_types:  # is Arnold loaded?
        ...
        if expected_value > len(ai_shadow_casting_lights) > inbetween_value:
            ...
            patch_message = (
                'Your scene contains "'
                + str(len(ai_shadow_casting_lights))
        ...
        if expected_value > len(ai_shadow_casting_lights) > inbetween_value:
            ...
            string_status = (
                ...
                + ' issue found. Your scene contains "'
                + str(len(ai_shadow_casting_lights))
                + '" Arnold shadow casting lights, you '
...
def check_default_object_names():
    ...
    if len(possible_offenders) == 1:
        patch_message_warning = (
            str(len(possible_offenders)) + " object contains a string extremely similar to the default names."
            "\n(Ignore this warning if the name describes your object properly)"
        )
    else:
        patch_message_warning = (
            str(len(possible_offenders)) + " objects contain a string extremely similar to the default names."
            "\n(Ignore this warning if the name describes your object properly)"
        )

    if len(offending_objects) == 1:
        patch_message_error = (
            str(len(offending_objects)) + " object was not named properly. \nPlease rename your objects descriptively."
        )
    else:
        patch_message_error = (
            str(len(offending_objects))
            + " objects were not named properly. \nPlease rename your objects descriptively."
        )
    ...
    if len(possible_offenders) != 0 and len(offending_objects) == 0:
        ...
        cmds.text(
            "output_" + item_id, e=True, l=str(len(offending_objects)) + " + [ " + str(len(possible_offenders)) + " ]"
        )
        patch_message = patch_message_error + "\n\n" + patch_message_warning
    ...
    def warning_default_object_names():
        ...
        if user_input == "Ignore Warning":
            cmds.button(
                "status_" + item_id, e=True, bgc=pass_color, l="", c=lambda args: warning_default_object_names()
            )
        else:
            cmds.button("status_" + item_id, e=True, l="")
    ...
    if issues_found > 0 or len(possible_offenders) > 0:
        ...
        for obj in offending_objects:
            string_status = (
                string_status + '"' + obj + '" was not named properly. Please rename ' "your object descriptively.\n"
            )
...
def check_objects_assigned_to_lambert1():
    ...
    if len(lambert1_objects) == 0:
        ...
        cmds.button(
            "status_" + item_id, e=True, bgc=error_color, l="?", c=lambda args: warning_objects_assigned_to_lambert1()
        )
    ...
    if len(lambert1_objects) == 1:
        patch_message = (
            str(len(lambert1_objects)) + " object is assigned to lambert1. \nMake sure no objects are "
            "assigned to lambert1.\n\n(Too see a list of "
            "objects, generate a full report)"
        )
    else:
        patch_message = (
            str(len(lambert1_objects)) + " objects are assigned to lambert1. \nMake sure no objects are "
    ...
    if issues_found > 0:
        ...
        for obj in lambert1_objects:
            string_status = (
                string_status + '"' + obj + '"  is assigned to lambert1. ' "It should be assigned to another shader.\n"
            )
        string_status = string_status[:-1]
    else:
...
def check_ngons():
    ...
    ngon_mel_command = (
        'string $ngons[] = `polyCleanupArgList 3 { "1","2","1","0","1","0","0","0","0",'
    ...
    if len(ngons_list) == 1:
        patch_message = (
            str(len(ngons_list)) + " ngon found in your scene. \nMake sure no faces have more than 4 "
            "sides.\n\n(Too see a list of objects, generate a full report)"
        )
    else:
        patch_message = (
            str(len(ngons_list)) + " ngons found in your scene. \nMake sure no faces have more than 4 "
            "sides.\n\n(Too see a list of objects, generate a full report)"
        )
...
def check_non_manifold_geometry():
    ...
    if len(nonmanifold_geo) == 1:
        patch_message = (
            str(len(nonmanifold_geo)) + " object with non-manifold geometry was found in your scene. "
            "\n\n(Too see a list of objects, generate a full report)"
        )
    else:
        patch_message = (
            str(len(nonmanifold_geo)) + " objects with non-manifold geometry were found in your scene. "
            "\n\n(Too see a list of objects, generate a full report)"
        )
...
def check_empty_uv_sets():
    ...
    if len(objects_extra_empty_uv_sets) == 0:
        cmds.button(
            "status_" + item_id, e=True, bgc=pass_color, l="", c=lambda args: print_message("No empty UV sets.")
        )
        issues_found = 0
    else:
    ...
    if len(objects_extra_empty_uv_sets) == 1:
        patch_message = (
            str(len(objects_extra_empty_uv_sets)) + " object found containing multiple UV Sets and empty UV Sets. "
            "\n\n(Too see a list of objects, generate a full report)"
        )
    else:
        patch_message = (
            str(len(objects_extra_empty_uv_sets)) + " objects found containing multiple UV Sets and empty UV Sets. "
            "\n\n(Too see a list of objects, generate a full report)"
        )
...
def check_frozen_transforms():
    ...
    for transform in all_transforms:
        ...
        for child in children:
            ...
            if object_type == "mesh" or object_type == "nurbsCurve":
                if (
                    cmds.getAttr(transform + ".rotateX") != 0
                    or cmds.getAttr(transform + ".rotateY") != 0
                    or cmds.getAttr(transform + ".rotateZ") != 0
                ):
                    if (
                        len(cmds.listConnections(transform + ".rotateX") or []) == 0
                        and len(cmds.listConnections(transform + ".rotateY") or []) == 0
                        and len(cmds.listConnections(transform + ".rotateZ") or []) == 0
                        and len(cmds.listConnections(transform + ".rotate") or []) == 0
                    ):
    ...
    if len(objects_no_frozen_transforms) == 1:
        patch_message = (
            str(len(objects_no_frozen_transforms)) + " object has un-frozen transformations. \n\n"
            "(Too see a list of objects, generate a full report)"
        )
    else:
        patch_message = (
            str(len(objects_no_frozen_transforms)) + " objects have un-frozen transformations. \n\n"
            "(Too see a list of objects, generate a full report)"
        )
...
def check_non_deformer_history():
    ...
    if len(possible_objects_non_deformer_history) == 1:
        patch_message_warning = (
            str(len(possible_objects_non_deformer_history)) + " object contains deformers often used for modeling.\n"
        )
    else:
        patch_message_warning = (
            str(len(possible_objects_non_deformer_history)) + " objects contain deformers often used for modeling.\n"
        )
    ...
    if len(possible_objects_non_deformer_history) != 0 and len(objects_non_deformer_history) == 0:
        ...
        cmds.text(
            "output_" + item_id,
            e=True,
            l=str(len(objects_non_deformer_history)) + " + [ " + str(len(possible_objects_non_deformer_history)) + " ]",
        )
        patch_message = patch_message_error + "\n\n" + patch_message_warning
        # return_message = patch_message_error + '\n' + patch_message_warning
        buttons_to_add.append("Select Objects With Suspicious Deformers")
...
def check_textures_color_space():
    ...
    float3_to_float_exceptions = {"RedshiftBumpMap": "input", "RedshiftDisplacement": "texMap"}
            ...
            for in_con in input_node_connections:
                ...
                if should_be_checked:
                    ...
                    if (
                        data_type == "float3"
                        and (
                            node_type in float3_to_float_exceptions
                            and node_in_con in float3_to_float_exceptions.values()
                        )
                        is False
                    ):
    ...
    if len(possible_objects_wrong_color_space) == 1:
        patch_message_warning = (
            str(len(possible_objects_wrong_color_space)) + " file node is using a color space "
            "that might not be appropriate for "
            "its connection.\n"
        )
    else:
        patch_message_warning = (
            str(len(possible_objects_wrong_color_space)) + " file nodes are using a color space "
            "that might not be appropriate for "
            "its connection.\n"
        )
    ...
    if len(objects_wrong_color_space) == 1:
        patch_message_error = (
            str(len(objects_wrong_color_space)) + " file node is using a color space that is not "
            "appropriate for its connection.\n"
        )
    else:
        patch_message_error = (
            str(len(objects_wrong_color_space)) + " file nodes are using a color space that is "
    ...
    if issues_found > 0 or len(possible_objects_wrong_color_space) > 0:
        ...
        for obj in objects_wrong_color_space:
            string_status = (
                ...
                + '"'
                + obj[0]
                + '" is using a color space ('
                + cmds.getAttr(obj[0] + ".colorSpace")
...
def check_other_network_paths():
    ...
    def check_paths(
        ...
        try:
            ...
            logger.debug(str(e))
            print(
                'Something went wrong when checking the attribute "'
                + path_attribute_name
                + '" in the nodes of type "'
                + str(node_type)
                + '".'
            )
    ...
    if issues_found > 0:
        ...
        for bad_node in incorrect_path_nodes:
            string_status = (
                ...
                + '" of the type "'
                + bad_node[1]
                + "\" isn't pointing to the the network drive. Your paths should be sourced from the network.\n"
            )
    else:
...
def settings_import_state():
    ...
    file_name = (
        cmds.fileDialog2(
            fileFilter=script_name + " Settings (*.txt)",
            dialogStyle=2,
            fileMode=1,
            okCaption="Import",
            caption='Importing Settings for "' + script_name + '"',
        )
        or []
    )
...
def settings_export_state():
    ...
    file_name = (
        cmds.fileDialog2(
            ...
            dialogStyle=2,
            okCaption="Export",
            caption='Exporting Settings for "' + script_name + '"',
        )
        or []
    )
