...
# Script Name
script_name = "Make IK Stretchy"

# Version:
script_version = "?.?.?"  # Module version (init)

# Settings
gt_make_ik_stretchy_settings = {"ik_handle": "", "attr_holder": ""}
...
def build_gui_make_ik_stretchy():
    ...
    # Build UI
    window_gui_make_ik_stretchy = cmds.window(
        window_name,
    ...
    attr_holder_status = cmds.button(
        l="Not loaded yet",
        bgc=(0.2, 0.2, 0.2),
        w=130,
        c=lambda x: select_existing_object(gt_make_ik_stretchy_settings.get("attr_holder")),
    )
    ...
    cmds.button(l="Make Stretchy", bgc=(0.6, 0.6, 0.6), c=lambda x: validate_operation())
    cmds.separator(h=10, style="none")  # Empty Space
    ...
    icon = ui_qt.QtGui.QIcon(":/ikSCsolver.svg")
    ...
    def object_load_handler(operation):
        ...
        if operation == "ik_handle":
            ...
            if received_valid_element:
                cmds.button(
                    ik_handle_status,
                    l=gt_make_ik_stretchy_settings.get("ik_handle"),
                    e=True,
                    bgc=(0.6, 0.8, 0.6),
                    w=130,
                )
            else:
                cmds.button(ik_handle_status, l="Failed to Load", e=True, bgc=(1, 0.4, 0.4), w=130)
        ...
        if operation == "attr_holder":
            ...
            if len(current_selection) == 0:
                cmds.warning(
                    "Nothing selected. Assuming you don't want an attribute holder. "
            ...
            if received_valid_element:
                cmds.button(
                    attr_holder_status,
                    l=gt_make_ik_stretchy_settings.get("attr_holder"),
                    e=True,
                    bgc=(0.6, 0.8, 0.6),
                    w=130,
                )
            else:
                cmds.button(attr_holder_status, l="Not provided", e=True, bgc=(0.2, 0.2, 0.2), w=130)
    ...
    def validate_operation():
        ...
        if gt_make_ik_stretchy_settings.get("ik_handle") == "":
            ...
            if cmds.objExists(gt_make_ik_stretchy_settings.get("ik_handle")):
                ...
                cmds.warning(
                    '"' + str(gt_make_ik_stretchy_settings.get("ik_handle")) + "\" couldn't be located. "
        ...
        if is_valid:
            if gt_make_ik_stretchy_settings.get("attr_holder") != "":
                if cmds.objExists(gt_make_ik_stretchy_settings.get("attr_holder")):
                    ...
                    cmds.warning(
                        '"' + str(gt_make_ik_stretchy_settings.get("attr_holder")) + "\" couldn't be located. "
                        "Make sure you didn't rename or deleted the object after loading it. "
        ...
        if is_valid:
            if stretchy_name:
                make_stretchy_ik(
                    gt_make_ik_stretchy_settings.get("ik_handle"),
                    stretchy_name=stretchy_name,
                    attribute_holder=attr_holder,
                )
            else:
                make_stretchy_ik(
                    gt_make_ik_stretchy_settings.get("ik_handle"), stretchy_name="temp", attribute_holder=attr_holder
                )
...
def build_gui_help_make_stretchy_ik():
    ...
    cmds.text(
        l="Use this button to load your ikHandle.\nThe joints will be automatically extracted from it.", align="center"
    )
    cmds.separator(h=10, style="none")  # Empty Space

    cmds.text(l="Load Attribute Holder:", align="center")
    cmds.text(
        l="Use this button to load your attribute holder.\nThis is usually a control.\n"
...
def make_stretchy_ik(ik_handle, stretchy_name="temp", attribute_holder=None):
    ...
    for index in range(len(ik_handle_joints)):
        ...
        distance_node = cmds.rename(
            distance_node, stretchy_name + "_defaultTerm" + int_to_en(index + 1).capitalize() + "_strechyDistanceShape"
        )
        distance_node_transform = cmds.rename(
            distance_node_transform,
            stretchy_name + "_defaultTerm" + int_to_en(index + 1).capitalize() + "_strechyDistance",
        )
        start_loc = cmds.rename(
            distance_node_locators[0], stretchy_name + "_defaultTerm" + int_to_en(index + 1).capitalize() + "_start"
        )
        end_loc = cmds.rename(
            distance_node_locators[1], stretchy_name + "_defaultTerm" + int_to_en(index + 1).capitalize() + "_end"
        )
    ...
    cmds.connectAttr(
        "%s.outColorR" % nonzero_stretch_condition_node, "%s.firstTerm" % stretch_condition_node
    )  # Distance One
    ...
    if attribute_holder:
        if cmds.objExists(attribute_holder):
            ...
            cmds.setAttr(attribute_holder + ".baseVolumeMultiplier", 0.5)
            ...
            cmds.setAttr(attribute_holder + ".minimumVolume", 0.4)
            ...
            volume_normalization_divide_node = cmds.createNode(
                "multiplyDivide", name=stretchy_name + "_volumeNormalization_divide"
            )
            ...
            cmds.connectAttr(
                "%s.outColorR" % nonzero_stretch_condition_node, "%s.input2X" % volume_normalization_divide_node
            )  # Distance One
            ...
            cmds.connectAttr(
                "%s.baseVolumeMultiplier" % attribute_holder, "%s.attributesBlender" % volume_base_blend_node
            )
