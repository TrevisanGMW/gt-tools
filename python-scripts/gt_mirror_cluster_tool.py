...
def build_gui_mirror_cluster_tool():
    ...
    cmds.radioButtonGrp("mirror_axis_btn_grp", nrb = 3,sl = 1, l = "Mirror Axis", labelArray3 = ["X","Y","Z"], columnAlign4 = ["center","left","left","left"],cw4 = [120,60,60,60])
    cmds.separator(h=10, style='none') # Spacing Before Next Container
    ...
    def update_stored_objects(expected_type):
        ...
        # If mesh
        if expected_type is "mesh" and received_valid_object == True:
            global_settings["loaded_mesh"] = received_object
            cmds.button(mesh_status, l=received_object, e=True, bgc=(.6, .8, .6), c=lambda x:loader_existance_check(global_settings.get("loaded_mesh")))
        elif expected_type is "mesh":
            cmds.button(mesh_status, l ="Failed to Load",e=True, bgc=(1, .4, .4), w=130,c="cmds.headsUpMessage( 'Make sure you select only one mesh and try again', verticalOffset=150 , time=5.0)")
        # If clusterHandle
        if expected_type is "clusterHandle" and received_valid_object == True:
            global_settings["loaded_cluster_handle"] = received_object
            cmds.button(cluster_handle_status, l=received_object, e=True, bgc=(.6, .8, .6), c=lambda x:loader_existance_check(global_settings.get("loaded_cluster_handle")))
        elif expected_type is "clusterHandle":
            cmds.button(cluster_handle_status, l ="Failed to Load",e=True, bgc=(1, .4, .4), w=130,c="cmds.headsUpMessage( 'Make sure you select a cluster and try again', verticalOffset=150 , time=5.0)")
...
def mirror_cluster():
    ...
    if is_current_setup_valid:  
        ...
        verticies_selected_mesh = get_cluster_verticies_on_mesh(mesh_transform, cluster_handle)
        mirrored_vertices = []
        ...
        for vertex in verticies_selected_mesh:
            ...
            if mirror_axis == 1:
                ...
                cmds.setAttr((closest_point_node + ".inPosition"),-vertex_position[0],vertex_position[1],vertex_position[2])
            elif mirror_axis == 2:
                cmds.setAttr((closest_point_node + ".inPosition"),vertex_position[0],-vertex_position[1],vertex_position[2])
            else:
                cmds.setAttr((closest_point_node + ".inPosition"),vertex_position[0],vertex_position[1],-vertex_position[2])
            ...
            mirrored_vertex = (mesh_transform + ".vtx[" + str(cmds.getAttr((closest_point_node + ".closestVertexIndex"))) + "]") # Find mirrored vertex
            vertex[0] = mirrored_vertex # Replace previous pair vertex with new found one
            mirrored_vertices.append(mirrored_vertex)
            cmds.delete(closest_point_node) # Delete utility node

        cluster_deformer_node = cmds.listConnections((cluster_handle + ".worldMatrix[0]"), type = "cluster", destination = 1)
        is_relative = cmds.getAttr((cluster_deformer_node[0] + ".relative"))
        
        new_cluster_name = cluster_handle.replace(cmds.textField("search_text_field", q = True, text=True), cmds.textField("replace_text_field", q = True, text=True))
        ...
        for num in range (len(verticies_selected_mesh)):
            cmds.percent(new_cluster[0], verticies_selected_mesh[num][0], v = verticies_selected_mesh[num][1])
...
@param mesh_transform: a selected mesh, for example "pSphere1"
@param cluster_handle: a cluster handle that has influence over the mesh
@return: a list of paired verticies and weights [vertex, weight]
"""
def get_cluster_verticies_on_mesh(mesh_transform, cluster_handle):
    cluster_deformer_node = cmds.listConnections((cluster_handle + ".worldMatrix[0]"), type ="cluster", destination = 1)
    cluster_set = cmds.listConnections(cluster_deformer_node[0], type = "objectSet" ) 
    ...
    # 46: Lattice Points
    extracted_verts = cmds.filterExpand(extracted_verts, selectionMask = (28,31,36,46)) # Filter Points Only
...
"""
Loader existance check. 
If object exists, select it.

@param obj: any Maya node
"""
def loader_existance_check(obj):
