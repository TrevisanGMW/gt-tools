...
def equidistant_constraints(start, end, target_list, skip_start_end=True, constraint=ConstraintTypes.PARENT):
    ...
    if not _func:
        logger.warning(f'Unable to create equidistant constraints. '
...
def constraint_targets(source_driver, target_driven, constraint_type=ConstraintTypes.PARENT,
                       maintain_offset=True, inter_type=0, rename_constraint=True, **kwargs):
    """
