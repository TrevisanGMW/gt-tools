...
def get_curve_file_path(file_name):
    """
    Get the path to a curve data file. This file should exist inside the core/data/curves folder.
    Args:
...
def get_curve(file_name, curve_dir=None):
    """
    Get the curve object from the path to a curve data file. This file should exist inside the core/data/curves folder.
    Args:
...
def combine_curves_list(curve_list, convert_bezier_to_nurbs=True):
    ...
    try:
        ...
        core_attr.freeze_channels(list(valid_curve_transforms))
        # Re-parent Shapes
...
class Curve:
    ...
    def get_data_as_dict(self):
        ...
        if self.transform:
            ...
            transform_data = {"position": pos_data, "rotation": rot_data, "scale": sca_data}
        curve_data = {
            "name": self.name,
    ...
    def set_transform(self, transform):
        ...
        if not transform or not isinstance(transform, Transform):
            logger.warning(
                f"Unable to set curve transform. " f'Must be a "Transform" object, but got "{str(type(transform))}".'
            )
            return
        self.transform = transform
...
class CurveShape:
    ...
    def read_data_from_existing_curve_shape(self, crv_shape):
        ...
        if cmds.objectType(crv_shape) not in CURVE_TYPES:
            logger.warning(
                f"Unable to extract curve data. Missing acceptable curve shapes. "
...
def write_curve_files_from_selection(
    target_dir=None, projection_axis=None, projection_scale=None, projection_fit=None, overwrite=True
):
    """
...
def generate_package_curve_thumbnail(target_dir, curve, image_format="jpg", line_width=5, rgb_color=(1, 1, 0.1)):
    ...
    from gt.core.color import set_color_viewport
...
def print_code_for_crv_files(target_dir=None, ignore_private=True, use_output_window=False):
    ...
    Args:
        target_dir (str, optional): If provided, this path will be used instead of the default "core/data/curves" path.
...
def add_shape_scale_cluster(obj, scale_driver_attr, reset_pivot=True):
    ...
    if not cluster:
        ...
        if reset_pivot:
            set_attr(
                obj_list=cluster_handle,
                attr_list=["scalePivotX", "scalePivotY", "scalePivotZ", "rotatePivotX", "rotatePivotY", "rotatePivotZ"],
                value=0,
            )
