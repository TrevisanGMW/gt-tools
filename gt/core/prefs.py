...
from gt.core.io import write_json, read_json_dict, write_data, delete_paths
...
class Prefs:
    ...
    def get_user_file(self, file_name, verbose=False):
        ...
        if not os.path.exists(_sub_folder):
            if verbose:
                logger.warning(
                    f"Unable to retrieve user file. " f'User file sub-folder does not exist. Path: "{_sub_folder}".'
                )
            return
    ...
    def get_all_user_files(self, verbose=False):
        ...
        if not os.path.exists(_sub_folder):
            if verbose:
                logger.warning(
                    f"Unable to retrieve user files. " f'User file sub-folder does not exist. Path: "{_sub_folder}".'
                )
            return files_dict
...
def toggle_skip_menu_creation():
    ...
    prefs.save()
    feedback = FeedbackMessage(
        intro="Skipping Menu Creation set to:",
        conclusion=str(inverted_state),
        style_conclusion="color:#FF0000;text-decoration:underline;",
    )
    feedback.print_inview_message()
...
def purge_package_settings():
    ...
    feedback = FeedbackMessage(
        intro="Package preferences were",
        conclusion="purged",
        suffix=".",
        style_conclusion="color:#FF0000;text-decoration:underline;",
    )
