...
def set_persistent_settings_maya_to_discord(
...
def build_gui_maya_to_discord():
    ...
    # Build UI
    window_gui_maya_to_discord = cmds.window(
        window_name,
    ...
    cmds.rowColumnLayout(
        nc=4, cw=[(1, 10), (2, 150), (3, 60), (4, 40)], cs=[(1, 10), (2, 0), (3, 0)], p=content_main
    )  # Title Column
    ...
    cmds.rowColumnLayout(
        nc=3, cw=[(1, 100), (2, 50), (3, 100), (4, 50)], cs=[(1, 10), (2, 0), (3, 0), (4, 0)], p=content_main
    )
    ...
    if gt_mtod_settings["is_first_time_running"] is True:
        ...
        if "Error" in gt_mtod_settings.get("discord_webhook_name") or "Missing Webhook" in gt_mtod_settings.get(
            ...
            cmds.text(webhook_name_text, e=True, l=gt_mtod_settings.get("discord_webhook_name"), bgc=[0.5, 0, 0])
        else:
    ...
    send_message_btn = cmds.iconTextButton(
        style="iconAndTextHorizontal",
        image1=send_message_btn_ico,
    ...
    screenshot_btn_color = [0.3, 0.3, 0.35]
    cmds.rowColumnLayout(nc=1, cw=[(1, 260), (2, 1), (3, 5)], cs=[(1, 10), (2, 0), (3, 0)], p=content_main)
    send_desktop_btn = cmds.iconTextButton(
        style="iconAndTextVertical",
        image1=send_desktop_btn_ico,
    ...
    send_maya_window_btn = cmds.iconTextButton(
        style="iconAndTextVertical",
        image1=send_maya_window_btn_ico,
    ...
    send_viewport_btn = cmds.iconTextButton(
        style="iconAndTextVertical",
        image1="hypershadeOutlinerPerspLayout.png",
        label="Send Viewport",
    ...
    objects_btn_color = [0.25, 0.3, 0.35]
    cmds.separator(h=5, style="none")  # Empty Space
    cmds.rowColumnLayout(nc=1, cw=[(1, 260), (2, 1), (3, 5)], cs=[(1, 10), (2, 0), (3, 0)], p=content_main)
    send_playblast_btn = cmds.iconTextButton(
        style="iconAndTextVertical",
        image1=send_playblast_btn_ico,
        label="Send Playblast",
        statusBarMessage="This button will record a playblast and attempt to " "send it to Discord.",
        olc=[1, 0, 0],
        enableBackground=True,
        bgc=objects_btn_color,
        h=80,
    ...
    cmds.rowColumnLayout(nc=2, cw=[(1, 128), (2, 128), (3, 5)], cs=[(1, 10), (2, 4), (3, 0)], p=content_main)
    send_obj_btn = cmds.iconTextButton(
        style="iconAndTextVertical",
        image1=send_obj_btn_ico,
        label="Send OBJ",
    ...
    send_fbx_btn = cmds.iconTextButton(
        style="iconAndTextVertical",
        image1=send_fbx_btn_ico,
        label="Send FBX",
    ...
    def update_text_status(error=False):
        ...
        if not error:
            ...
            cmds.text(status_message_text, e=True, l="...", bgc=(0.5, 0, 0))
            cmds.text(status_code_text, e=True, l="Error", bgc=(0.5, 0, 0))
            ...
            if status_value in success_codes:
                try:
                    ...
                    if upload_message.strip() != "":
                        ...
                        response_inview_feedback(
                            operation_name, response, display_inview=gt_mtod_settings.get("feedback_visibility")
                        )
            ...
            try:
                ...
                def threaded_upload():
                    ...
                    try:
                        utils.executeDeferred(disable_buttons)
                        response = discord_post_attachment(
                            get_username(), upload_message, temp_img_file, gt_mtod_settings.get("discord_webhook")
                        )
                        utils.executeDeferred(enable_buttons)
                        utils.executeDeferred(parse_sending_response, response)
    ...
    def send_animated_playblast():
        ...
        if gt_mtod_settings.get("is_webhook_valid"):
            try:
                ...
                temp_playblast_file = capture_playblast_animation(
                    temp_path,
                    gt_mtod_settings.get("video_scale_pct"),
    ...
    icon = ui_qt.QtGui.QIcon(icon_image)
...
def build_gui_settings_maya_to_discord():
    ...
    cmds.rowColumnLayout(nc=1, cw=[(1, 310)], cs=[(1, 10)], p=main_column)  # Window Size Adjustment
    cmds.rowColumnLayout(nc=1, cw=[(1, 300)], cs=[(1, 10)], p=main_column)  # Title Column
    cmds.text(script_name + " Settings", bgc=[0.4, 0.4, 0.4], fn="boldLabelFont", align="center")
    cmds.separator(h=10, style="none", p=main_column)  # Empty Space
    ...
    cmds.separator(h=5, style="none")  # Empty Space
    new_webhook_input = cmds.textField(
        pht="https://discordapp.com/api/webhooks/...", text=current_webhook, font="smallPlainLabelFont"
    )
    ...
    new_username_input = cmds.textField(
        pht="username (not required)", text=current_custom_username, font="smallPlainLabelFont"
    )
    ...
    video_scale_input = cmds.intSliderGrp(
        field=True,
        minValue=1,
        maxValue=100,
        fieldMinValue=1,
        fieldMaxValue=100,
        value=current_video_scale,
        cw=([1, 35], [2, 65]),
    )
    ...
    feedback_visibility_chk = cmds.checkBox(
        label="", value=gt_mtod_settings.get("feedback_visibility"), cc=lambda args: update_checkbox_settings_data()
    )
    cmds.text(l="Display Viewport Feedback", align="left", font="smallPlainLabelFont")
    timestamp_visibility_chk = cmds.checkBox(
        label="", value=gt_mtod_settings.get("timestamp_visibility"), cc=lambda args: update_checkbox_settings_data()
    )
    ...
    cmds.rowColumnLayout(nc=1, cw=[(1, 300)], cs=[(1, 10)], p=main_column)
    cmds.button(l="Apply", h=30, bgc=(0.6, 0.6, 0.6), c=lambda args: apply_settings())
    cmds.separator(h=8, style="none")
    ...
    qw = OpenMayaUI.MQtUtil.findWindow(window_name)
    widget = ui_qt.shiboken.wrapInstance(int(qw), ui_qt.QtWidgets.QWidget)
    icon = ui_qt.QtGui.QIcon(":/toolSettings.png")
    widget.setWindowIcon(icon)
    ...
    def apply_settings():
        ...
        set_persistent_settings_maya_to_discord(
            cmds.textField(new_username_input, q=True, text=True),
...
def capture_desktop_screenshot(image_file):
    ...
    ptr = ui_qt.shiboken.wrapInstance(int(win), ui_qt.QtWidgets.QMainWindow)
    screen_number = app.desktop().screenNumber(ptr)
    screen_geometry = app.desktop().screenGeometry(screen_number)
    frame = app.primaryScreen().grabWindow(
        0, screen_geometry.x(), screen_geometry.y(), screen_geometry.width(), screen_geometry.height()
    )
...
def capture_app_window(image_file):
    ...
    frame = ui_qt.QtGui.QPixmap.grabWindow(long_win_id)
    frame.save(image_file)
    return image_file
...
def discord_post_message(username, message, webhook_url):
    ...
    """
    if python_version == 3:
        bot_message = {"username": username, "content": message}
...
def discord_post_attachment(username, message, file_path, webhook_url):
    ...
    if python_version == 3:
        ...
        response = http_obj.request(uri=webhook_url, method="POST", headers=headers, body=data)
...
def capture_playblast_animation(video_file, scale_pct, compression, video_format):
    ...
    playblast = cmds.playblast(
        p=scale_pct, f=video_file, compression=compression, format=video_format, forceOverwrite=True, v=False
    )
    file_size = os.path.getsize(playblast)
    OpenMayaUI.M3dView.active3dView().portHeight()
    message = (
        '<span style="color:#FFFFFF;">Playblast File Size:'
        '</span> <span style="color:#FF0000;text-decoration:underline;">'
        + get_readable_size(file_size, precision=2)
        + "</span>"
    )
    if gt_mtod_settings.get("feedback_visibility"):
        cmds.inViewMessage(amg=message, pos="botLeft", fade=True, alpha=0.9)
        cmds.inViewMessage(
            amg='<span style="color:#FFFFFF;">Open the the script editor for more information',
            pos="botLeft",
            fade=True,
            alpha=0.9,
        )
        ...
        if status_value in success_codes:
            ...
            if write_output:
                sys.stdout.write(
                    "The "
                    + str(operation_name)
                    + " was sent successfully.  Web response: "
                    + str(reason_value)
                    + " ("
                    + str(status_value)
                    + ")"
            ...
            message += (
                "The " + str(operation_name) + ' was <span style="color:#FF0000;text-decoration:underline;">not sent.'
            )
            if write_output:
                sys.stdout.write(
                    "The "
                    + str(operation_name)
                    + " was sent.  Web response: "
                    + str(reason_value)
                    + " ("
                    + str(status_value)
                    + ")"
