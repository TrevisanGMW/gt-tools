...
from gt.core.iterable import sanitize_maya_list, filter_list_by_type
from gt.core.attr import hide_lock_default_attrs, set_trs_attr
from gt.core.color import set_color_viewport, ColorConstants
from gt.core.transform import match_transform
from gt.core.math import get_bbox_position
from gt.core.naming import NamingConstants
from gt.core.node import Node
from gt.core import hierarchy
...
def create_surface_from_object_list(obj_list, surface_name=None, degree=3, custom_normal=None):
    ...
    offset_distance = 2
    crv_pos = cmds.offsetCurve(
        crv_mid,
        name=f"{crv_mid}PositiveOffset",
        distance=offset_distance,
        constructionHistory=True,
        **normal_parameters,
    )[0]
    crv_neg = cmds.offsetCurve(
        crv_mid,
        name=f"{crv_mid}NegativeOffset",
        distance=-offset_distance,
        constructionHistory=True,
        **normal_parameters,
    )[0]
...
def multiply_surface_spans(input_surface, u_multiplier=0, v_multiplier=0, u_degree=None, v_degree=None):
    ...
    if cmds.objectType(input_surface) == "transform":
        ...
        shapes = cmds.listRelatives(input_surface, shapes=True, typ=SURFACE_TYPE, fullPath=True)
        if shapes:
            input_surface = shapes[0]
        else:
            logger.debug(f"Unable to multiply surface division. " f'No "nurbsSurface" found in the provided transform.')
            return
...
def create_follicle(input_surface, uv_position=(0.5, 0.5), name=None):
    ...
    if cmds.objectType(input_surface) == "transform":
        ...
        if shapes:
            ...
            logger.debug(f"Unable create follicle. " f'No "nurbsSurface" found in the provided transform.')
            return
    if cmds.objectType(input_surface) != SURFACE_TYPE:
        logger.debug(f"Unable create follicle. " f"The provided input surface is not a {SURFACE_TYPE}.")
        return
    if not name:
        name = "follicle"
...
class Ribbon:
    ...
    def set_bind_joint_data(self, orient_offset=None, parenting=None):
        ...
        if orient_offset:
            if not isinstance(orient_offset, tuple) or len(orient_offset) < 3:
                logger.debug(
                    f"Unable to set bind joint orient offset. "
                    f"Invalid input. Must be a tuple with X, Y and Z values."
                )
                return

            if not all(isinstance(num, (int, float)) for num in orient_offset):
                logger.debug(f"Unable to set bind joint orient offset. " f"Input must contain only numbers.")
                return
    ...
    def _get_or_create_surface(self, prefix):
        ...
        if isinstance(self.sur_data, (list, tuple)):
            ...
            if _filter_obj_list:
                _obj_list = sanitize_maya_list(
                    input_list=self.sur_data, sort_list=False, filter_unique=False, reverse_list=True
                )
                if not _obj_list or len(_obj_list) < 2:
                    logger.warning(
                        f"Unable to create surface using object list. "
                    ...
                    multiply_surface_spans(
                        input_surface=_sur, u_degree=1, v_degree=3, v_multiplier=self.sur_spans_multiplier
                    )
                    return _sur
            # Position List
            ...
            if _filter_pos_list:
                ...
                _sur = create_surface_from_object_list(obj_list=obj_list_locator, surface_name=f"{prefix}_ribbon_sur")
        ...
        surface = cmds.nurbsPlane(
            axis=(0, 1, 0), width=1, lengthRatio=24, degree=3, patchesU=1, patchesV=v_patches, constructionHistory=False
        )[0]
    ...
    def build(self):
        ...
        u_position_ctrls = get_positions_from_curve(
            curve=u_curve_for_positions, count=num_controls, periodic=is_periodic, space="uv"
        )
        u_position_joints = get_positions_from_curve(
            curve=u_curve_for_positions, count=num_joints, periodic=is_periodic, space="uv"
        )
        ...
        hierarchy.parent(source_objects=ribbon_ctrl, target_parent=ribbon_offset)
        hierarchy.parent(source_objects=control_grp, target_parent=ribbon_ctrl)
        hierarchy.parent(source_objects=[ribbon_offset, bind_grp, setup_grp], target_parent=parent_group)
        hierarchy.parent(source_objects=[input_surface, driver_joints_grp, follicles_grp], target_parent=setup_grp)
        ...
        for index in range(len(u_position_joints)):
            _fol_tuple = create_follicle(
                input_surface=str(surface_shape),
            ...
            if self.sur_data_is_driven and isinstance(self.sur_data, list):
                cmds.parentConstraint(
                    _follicle_transform, self.sur_data_sanitized[index], maintainOffset=self.sur_data_maintain_offset
                )
                continue
        ...
        hierarchy.parent(source_objects=bind_joints, target_parent=bind_grp)
        ...
        for index in range(num_controls):
            ...
            _follicle_transform = cmds.listRelatives(_follicle_shape, parent=True, fullPath=True)[0]
        ...
        for index in range(num_controls):
            ...
            hierarchy.parent(source_objects=ctrl, target_parent=ctrl_offset_grp)
            ...
            hierarchy.parent(source_objects=joint, target_parent=ctrl_jnt_ofs_grp)
        ...
        hierarchy.parent(source_objects=ctrl_offset_grps, target_parent=control_grp)
        hierarchy.parent(source_objects=ctrl_jnt_offset_grps, target_parent=driver_joints_grp)

        hide_lock_default_attrs(
            obj_list=ctrl_offset_grps + ctrl_jnt_offset_grps, translate=True, rotate=True, scale=True, visibility=False
        )
        ...
        bind_grp_children = cmds.listRelatives(bind_grp, children=True, fullPath=True)
...
if __name__ == "__main__":
    ...
    # from gt.core.control_utils import create_fk
    # test_fk_ctrls = create_fk(target_list=test_joints, constraint_joint=False)
    # Create Ribbon
    ribbon_factory = Ribbon(equidistant=True, num_controls=5, num_joints=8, add_fk=True)
