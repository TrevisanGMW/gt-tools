...
class Vector3:
    ...
    def set_from_tuple(self, values):
        ...
        if (
            ...
            and len(values) == 3
            and all(isinstance(coord, (int, float)) for coord in values)
        ):
...
class Transform:
    ...
    def __repr__(self):
        ...
        """
        return f"position={str(self.position)}, " f"rotation={str(self.rotation)}, " f"scale={str(self.scale)}"
    ...
    def set_from_tuple(self, position_tuple, rotation_tuple, scale_tuple):
        ...
        if (
            all(isinstance(coord, (int, float)) for coord in position_tuple)
            and all(isinstance(coord, (int, float)) for coord in rotation_tuple)
            and all(isinstance(coord, (int, float)) for coord in scale_tuple)
        ):
    ...
    def set_transform_from_object(self, obj_name, world_space=True):
        ...
        if world_space:
            ...
            position = core_attr.get_multiple_attr(obj_list=[obj_name], attr_list=["tx", "ty", "tz"], verbose=False)
            if position and len(position) == 3:
                self.set_position(xyz=list(position.values()))
            rotation = core_attr.get_multiple_attr(obj_list=[obj_name], attr_list=["rx", "ry", "rz"], verbose=False)
            if rotation and len(rotation) == 3:
                self.set_rotation(xyz=list(rotation.values()))
        scale = core_attr.get_multiple_attr(obj_list=[obj_name], attr_list=["sx", "sy", "sz"], verbose=False)
    ...
    def set_transform_from_dict(self, transform_dict):
        ...
        if transform_dict and not isinstance(transform_dict, dict):
            logger.debug(f"Unable to set transform from dictionary. " f"Invalid input, argument must be a dictionary.")
            return
        ...
        for data in [position, rotation, scale]:
            if not data or not isinstance(data, (tuple, list)) or len(data) != 3:
                logger.debug(
                    f"Unable to set transform from dictionary. "
    ...
    def apply_transform(self, target_object, world_space=True, object_space=False, relative=False):
        ...
        if world_space:
            ...
            core_attr.set_attr(attribute_path=f"{target_object}.sx", value=self.scale.x)
            core_attr.set_attr(attribute_path=f"{target_object}.sy", value=self.scale.y)
            core_attr.set_attr(attribute_path=f"{target_object}.sz", value=self.scale.z)
        else:
            position = self.position.get_as_tuple()
            rotation = self.rotation.get_as_tuple()
            scale = self.scale.get_as_tuple()
            core_attr.set_trs_attr(target_obj=target_object, value_tuple=position, translate=True)
            core_attr.set_trs_attr(target_obj=target_object, value_tuple=rotation, rotate=True)
            core_attr.set_trs_attr(target_obj=target_object, value_tuple=scale, scale=True)
    ...
    def get_transform_as_dict(self):
        ...
        """
        transform_dict = {
            "position": self.get_position(as_tuple=True),
...
def reset_transforms():
    ...
    if output_counter > 0:
        feedback = core_fback.FeedbackMessage(quantity=output_counter, conclusion="transforms were reset.")
        if output_counter == 1:
            feedback = core_fback.FeedbackMessage(
                intro=f'"{current_selection_short[0]}"',
...
def convert_transforms_to_locators():
    ...
    if counter > 0:
        ...
        feedback = core_fback.FeedbackMessage(
            quantity=counter, singular="locator was", plural="locators were", conclusion="created."
        )
        if counter == 1:
            feedback = core_fback.FeedbackMessage(
                intro=f'"{selection_short[0]}"',
...
def set_equidistant_transforms(start, end, target_list, skip_start_end=True, constraint="parent"):
    ...
    """
    constraints = core_cnstr.equidistant_constraints(
        start=start, end=end, target_list=target_list, skip_start_end=skip_start_end, constraint=constraint
    )
    if constraints:
...
def translate_shapes(obj_transform, offset):
    ...
    if not shapes:
        ...
        return
    for shape in shapes:
        from gt.core.hierarchy import get_shape_components
