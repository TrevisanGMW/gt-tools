...
from gt.tools.auto_rigger_legacy.rigger_biped_logic import *
from gt.tools.auto_rigger_legacy.rigger_data import *
from gt.ui import resource_library
from gt.tools.auto_rigger_legacy import rigger_corrective_logic
from gt.tools.auto_rigger_legacy import rigger_facial_logic
...
def import_facial_proxy_pose(source_path=None, source_dict=None):
    """
    Imports a JSON file containing the translation, rotation and scale data for every proxy curve
    (exported using the "export_proxy_pose" function)
    Uses the imported data to set the translation, rotation and scale position of every proxy curve
    ...
    if len(file_name) > 0:
        ...
        try:
            if source_dict:
                import_facial_proxy_pose_json(source_dict)
            else:
                with open(pose_file) as json_file:
                    data = json.load(json_file)
                    import_facial_proxy_pose_json(data)
...
def import_corrective_proxy_pose(source_path=None, source_dict=None):
    """
    Imports a JSON file containing the translation, rotation and scale data for every proxy curve
    (exported using the "export_proxy_pose" function)
    Uses the imported data to set the translation, rotation and scale position of every proxy curve
    ...
    if len(file_name) > 0:
        ...
        try:
            if source_dict:
                import_corrective_proxy_pose_json(source_dict)
            else:
                with open(pose_file) as json_file:
                    data = json.load(json_file)
                    import_corrective_proxy_pose_json(data)
...
def rebuild_rig():
    ...
    """
    from tools.auto_rigger_legacy import rigger_rebuild
