...
class ComplexCurve(Curve):
    def __init__(self, name=None, build_function=None):
        """
        Initializes a Curve object
        Args:
            name (str, optional): ComplexCurve transform name (shapes names are determined by the Callable function)
        """
    ...
    def build(self, *args, **kwargs):
        ...
        """
        if not self.is_curve_valid():
            logger.warning("ComplexCurve object is missing a callable function.")
            return
        try:
...
class ComplexCurves:
    def __init__(self):
        """
        A library of complex curve objects. These a created using a callable function.
        Use "build()" to create them in Maya.
        """
    from gt.utils.data.controls import cluster_driven
    scalable_arrow = ComplexCurve(build_function=cluster_driven.create_scalable_arrow)
