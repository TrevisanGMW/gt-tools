...
def capture_playblast_animation(video_file, scale_pct, compression, video_format):
    ...
    message = '<span style=\"color:#FFFFFF;\">Playblast File Size:</span> <span style=\"color:#FF0000;text-decoration:underline;\">' + get_readable_size(
        file_size, precision=2) + '</span>'
...
def discord_get_webhook_name(webhook_url):
    ...
    if python_version == 3:
        try:
            ...
            print(e)
            cmds.warning(
                'Error connecting to the provided webhook. Make sure you\'re pasting the correct URL and check your internet connection or firewall.')
...
def response_inview_feedback(operation_name, response, write_output=True, display_inview=True):
    ...
    if len(response) >= 1:
        ...
        if status_value in success_codes:
            message += 'The ' + str(
                operation_name) + ' was <span style=\"color:#00FF00;text-decoration:underline;\">sent successfully</span>.'
            if write_output:
            ...
            if write_output:
                sys.stdout.write(
                    'The ' + str(operation_name) + ' was sent.  Web response: ' + str(reason_value) + ' (' + str(
                        status_value) + ')')
    else:
        message += 'The ' + str(
            operation_name) + ' was <span style=\"color:#FF0000;text-decoration:underline;\">not sent.'
