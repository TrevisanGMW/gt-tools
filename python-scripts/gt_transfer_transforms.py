...
def build_gui_transfer_transforms():
    ...
    cmds.rowColumnLayout(nc=4, cw=[(1, 21),(2, 63),(3, 62),(4, 62)], cs=[(1,15),(2,0),(3,3),(4,3)], p=content_main)
...
def export_trs_transforms():
    ...
    if is_valid:
        file_name = cmds.fileDialog2(fileFilter=script_name + " - JSON File (*.json)", dialogStyle=2, okCaption= 'Export', caption= 'Exporting TRS for Selected Objects') or []
        if len(file_name) > 0:
    ...
    if successfully_created_file and is_valid:
        ...
        try: 
            ...
            cmds.inViewMessage(amg=unique_message + '<span style=\"color:#FF0000;text-decoration:underline;\">Transforms</span><span style=\"color:#FFFFFF;\"> exported.</span>', pos='botLeft', fade=True, alpha=.9)
...
def import_trs_transforms():
    ...
    file_name = cmds.fileDialog2(fileFilter=script_name + " - JSON File (*.json)", dialogStyle=2, fileMode= 1, okCaption= 'Import', caption= 'Importing Transforms for "' + script_name + '"') or []
            ...
            with open(pose_file) as json_file:
                ...
                try:
                    ...
                    if failed_imports:
                        ...
                        print('#' * 80)
                    unique_message = '<' + str(random.random()) + '>'
                    cmds.inViewMessage(amg=unique_message + '<span style=\"color:#FF0000;text-decoration:underline;\">Transforms</span><span style=\"color:#FFFFFF;\"> imported!</span>', pos='botLeft', fade=True, alpha=.9)
