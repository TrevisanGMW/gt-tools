...
class FeedbackMessage:
    ...
    def get_string_message(self):
        ...
        found_text = []
        for text in [self.prefix, self.intro, self._pluralization, self.conclusion, self.suffix]:
            if text:
    ...
    def get_inview_formatted_message(self):
        ...
        if self._quantity_str:
            if self.style_quantity:
                text_to_join.insert(
                    self.quantity_index, f'<span style="{str(self.style_quantity)}">{self._quantity_str}</span>'
                )
            else:
                text_to_join.insert(self.quantity_index, self._quantity_str)
        # Determine if zero overwriting - Overwrites when quantity is zero
        if self._overwrite_message:
            text_to_join = []
    ...
    def print_inview_message(
        self, position="botLeft", alpha=0.9, stay_time=2000, print_message=True, system_write=True
    ):
        """
        ...
        if not print_message:
            return
        cmds.inViewMessage(
            amg=self.get_inview_formatted_message(), position=position, fade=True, alpha=alpha, fadeStayTime=stay_time
        )
        if system_write:
