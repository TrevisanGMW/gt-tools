...
def cross_product_differences(vector_a, vector_b, vector_c):
    ...
    vector_a = OpenMaya.MVector([vector_a[0] - vector_b[0], vector_a[1] - vector_b[1], vector_a[2] - vector_b[2]])
    vector_b = OpenMaya.MVector([vector_c[0] - vector_b[0], vector_c[1] - vector_b[1], vector_c[2] - vector_b[2]])
...
def dist_path_sum(input_list):
    ...
    if num_elements < 2:
        logger.debug(
            "At least two elements are required to properly calculate distances. " "Distance is zero otherwise."
        )
    ...
    for index in range(num_elements - 1):
        ...
        if isinstance(element_a, (tuple, list)) and isinstance(element_b, (tuple, list)):
            distance = dist_xyz_to_xyz(
                element_a[0], element_a[1], element_a[2], element_b[0], element_b[1], element_b[2]  # A: XYZ
            )  # B: XYZ
        else:
            logger.warning(
                "Unsupported types detected. Total distance might not be accurate. "
