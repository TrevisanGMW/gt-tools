...
class Ribbon:
    ...
    def build_from_list(self, obj_list):
        """
        Build a ribbon rig.
        """
        num_controls = self.num_controls
        ...
        # Create Surface in case not provided or missing
        # input_surface = self._get_or_create_surface(prefix=prefix)
        input_surface = create_surface_from_object_list(obj_list=obj_list, surface_name=f"{prefix}ribbon_surface")
        input_surface = Node(input_surface)
        multiply_surface_spans(input_surface=input_surface, v_multiplier=4, v_degree=3)
...
def multiply_surface_spans(input_surface, u_multiplier=0, v_multiplier=0, u_degree=1, v_degree=1):
    ...
    if cmds.objectType(input_surface) == 'transform':
        ...
        shapes = cmds.listRelatives(input_surface, shapes=True, typ="nurbsSurface")
        if shapes:
...
if __name__ == "__main__":
    ...
    ribbon_factory.build_from_list(obj_list=["joint1", "joint2", "joint3", "joint4", "joint5", "joint6"])
