...
def build_gui_fspy_importer():
    ...
    # Build UI
    window_gui_fspy_importer = cmds.window(
        window_name,
    ...
    cmds.rowColumnLayout(
        nc=3, cw=[(1, 10), (2, 270), (3, 50)], cs=[(1, 10), (2, 0), (3, 0)], p=content_main
    )  # Title Column
    ...
    cmds.button(l="Import  (Generate Camera)", bgc=(0.6, 0.6, 0.6), c=lambda x: check_before_run())
    cmds.separator(h=10, style="none", p=body_column)  # Empty Space
    ...
    def check_before_run():
        ...
        if is_valid:
            gt_import_fspy_json(
                json_path,
                image_path,
                convert_up_axis_z_to_y=convert_z_to_y,
    ...
    def load_json_path():
        ...
        file_path = cmds.fileDialog2(
            fileFilter=multiple_filters, dialogStyle=2, fm=1, caption="Select fSpy JSON File", okc="Select JSON"
        )
    ...
    icon = ui_qt.QtGui.QIcon(ui_res_lib.Icon.tool_fspy_importer)
...
def build_gui_help_fspy_importer():
    ...
    cmds.text(
        l=script_name + " allows you import the data of a JSON\n file (exported out of fSpy) into Maya", align="center"
    )
    cmds.separator(h=10, style="none")  # Empty Space
    ...
    cmds.text(
        l="camera so it matches the position and rotation identified in fSpy.\n "
    ...
    cmds.separator(h=4, style="none")  # Empty Space
    cmds.text(
        l="Step 3: Load the files\nIn Maya, run the script and load your JSON and Image files",
        align="center",
        font=help_font,
    )
    cmds.separator(h=4, style="none")  # Empty Space
    ...
    cmds.text(
        l="Do not change the resolution of the image file or crop the image\nor it might not work properly.",
        align="center",
        font=help_font,
    )
    cmds.separator(h=10, style="none")  # Empty Space
    ...
    cmds.text(
        l="inside fSpy. This is because different software use different\n world coordinate systems.",
        align="center",
        font=help_font,
    )
    cmds.separator(h=10, style="none")  # Empty Space
...
def gt_import_fspy_json(
    json_path, image_path, convert_up_axis_z_to_y=True, lock_camera=True, set_scene_resolution=True
):
    """
    ...
    try:
        ...
        if convert_up_axis_z_to_y:
            ...
            message = (
                'Camera <span style="color:#FF0000;text-decoration:underline;"> '
        ...
        if set_scene_resolution:
            ...
            message = (
                'Scene resolution changed to: <span style="color:#FF0000;text-decoration:underline;">'
                + str(image_width)
                + "x"
                + str(image_height)
                + " </span>"
