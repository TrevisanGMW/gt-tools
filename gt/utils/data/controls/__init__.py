...
Similar to a curve, the output of a control is usually a nurbs element.
The difference is that a control is usually composed of multiple elements or extra logic.
For example, a control might have extra attributes that allow the shape to change or transform limits to affect it.

Note: All controls return a "ControlData" object as their return value.
Object names and other data can be retrieved through such object.
"""
...
class ControlConstants:
    def __init__(self):
        ...
        Control Constants
        """
    DIR_CURVES = os.path.join(DataDirConstants.DIR_CONTROLS, "ctrl_curves")
    # Naming
    SUFFIX_GRP = 'grp'
    SUFFIX_CRV = 'crv'
    SUFFIX_JNT = 'jnt'
...
def get_control_curve(curve_file_name):
    """
    Get the path to a resource file. This file should exist inside the resources' folder.
    Args:
        curve_file_name (str): Name of the file. It's not necessary to include the file extension ".crv".
    Returns:
        Curve or None: A Curve object loaded with the requested curve name. Curve(data_from_file=curve_file_path)
                       None if provided name is not found.
    """
    if not curve_file_name or not isinstance(curve_file_name, str):
        return
    if not curve_file_name.endswith('.crv'):
        curve_file_name = f'{curve_file_name}.crv'
    curve_file_path = os.path.join(ControlConstants.DIR_CURVES, curve_file_name)
    if os.path.exists(curve_file_path):
        return Curve(data_from_file=curve_file_path)
