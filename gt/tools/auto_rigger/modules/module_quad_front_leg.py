"""
Auto Rigger Quadruped Front Leg Module
"""
...
class ModuleQuadFrontLeg(tools_rig_frm.ModuleGeneric):
    __version__ = "0.0.1-alpha"
    icon = ui_res_lib.Icon.rigger_module_quad_front_leg
    allow_parenting = True
    allow_multiple = True

    def __init__(self, name="Quad Front Leg", prefix=None, suffix=None, create_twist_joints=True, ik_world=True):
        """
        Initializes the front leg module for a quadruped character rig.

        Args:
            name (str): The display name of the module in the rigging UI. Defaults to "Quad Front Leg".
            prefix (str, optional): A prefix to apply to all generated object names. Defaults to None.
            suffix (str, optional): A suffix to apply to all generated object names. Defaults to None.
            create_twist_joints (bool): If True, twist joints will be created for the upper and lower leg.
            ik_world (bool): If True, enables IK world space switching in the final rig. Defaults to True.
        """
        ...
        # Proxy Names
        scapula_name = "frontScapula"  # A.k.a shoulder blade
        upperleg_name = "frontUpperLeg"
        lowerleg_name = "frontLowerLeg"
        paw_name = "frontPaw"
        toes_name = "frontToes"
        ball_name = "frontBall"
        toesend_name = "frontToesEnd"
        # Pivots Names
        heel_name = "heel"
        ...
        # Proxy Trans
        pos_scapula = core_trans.Vector3(0, 68, 40)
        pos_upperleg = core_trans.Vector3(0, 60, 40)
        pos_lowerleg = core_trans.Vector3(0, 35.9, 36)
        pos_paw = core_trans.Vector3(0, 10.25, 36.6)
        pos_toes = core_trans.Vector3(0, 7.2, 37.42)
        pos_ball = core_trans.Vector3(0, 4.4, 39.12)
        pos_toesend = core_trans.Vector3(0, 0, 44)
        # Pivots Trans
        pos_heel = core_trans.Vector3(0, 0, 36)
        pos_bank_left = core_trans.Vector3(0, 0, 40)
        pos_bank_right = core_trans.Vector3(0, 0, 40)

        # Proxies
        self.scapula_proxy = tools_rig_frm.Proxy(name=scapula_name)
        self.scapula_proxy.start_position = pos_scapula
        self.scapula_proxy.set_initial_position(xyz=pos_scapula)
        self.scapula_proxy.set_locator_scale(scale=2)
        self.scapula_proxy.set_meta_purpose(value=scapula_name)
        self.scapula_proxy.set_rotation_order("yzx")
        self.scapula_proxy.add_driver_type(
            driver_type=[
    ...
    def setup_direction_curve(
        self, dir_curve, aim_object, world_up_object=None, up_vec=None, x_offset=None, skip_axis=None
    ):
        """
    ...
    def create_scp_joints(self):
        """Create joints needed to build an Ik solver to ensure the direction for the proxies of the scapula."""
        scapula_solver_joints = []
        _dir_curves = [self.scapula_dir_curve, self.upperleg_dir_curve]

        for dir_curve in _dir_curves:
            joint_name = f"{dir_curve.get_short_name()}_scp_jnt"
            joint = core_node.create_node(node_type="joint", name=joint_name)
            cmds.delete(cmds.parentConstraint(dir_curve, joint))
            if scapula_solver_joints:
                cmds.parent(joint, scapula_solver_joints[-1])
            scapula_solver_joints.append(joint)
        [cmds.makeIdentity(jnt, apply=True, rotate=True) for jnt in scapula_solver_joints]

        return scapula_solver_joints

    def setup_proxy_coplane(self, scapula_scp_joints, coplane_joints, pole_joints):
        """Create the setup the proxy coplane.
        Args:
            scapula_scp_joints (list): scapula solver joints
            coplane_joints (list): coplane joints
            pole_joints (list): pole joints
        """
        scapula_scp_prefix = f"{self._assemble_node_name(self.setup_name)}_scp"
        ...
        scapula_node = tools_rig_utils.find_proxy_from_uuid(self.scapula_proxy.get_uuid())
        upperleg_node = tools_rig_utils.find_proxy_from_uuid(self.upperleg_proxy.get_uuid())
        lowerleg_node = tools_rig_utils.find_proxy_from_uuid(self.lowerleg_proxy.get_uuid())
        paw_node = tools_rig_utils.find_proxy_from_uuid(self.paw_proxy.get_uuid())

        scapula_ik_handle = cmds.ikHandle(
            sj=scapula_scp_joints[0], ee=scapula_scp_joints[1], n=f"{scapula_scp_prefix}_ikHandle", sol="ikSCsolver"
        )[0]
        scapula_ik_handle = core_node.Node(scapula_ik_handle)
        cmds.parent(scapula_scp_joints[0], scapula_node)
        cmds.parent(scapula_ik_handle, upperleg_node)
        ...
        objs_to_hide.extend(scapula_scp_joints)
        objs_to_hide.append(coplane_ik_handle)
        objs_to_hide.append(pole_ik_handle)
        objs_to_hide.append(scapula_ik_handle)
    ...
    def align_proxies_to_joints(self):
        """
        Aligns proxies to their corresponding joints by matching translation.
        """
    ...
    def build_proxy(self, **kwargs):
        ...
        """
        self.scapula_proxy.parent_uuid = self.parent_uuid
        self.paw_proxy.parent_uuid = self.parent_uuid
        self.upperleg_proxy.set_parent_uuid(self.scapula_proxy.get_uuid())
    ...
    def build_proxy_setup(self):
        ...
        scapula_offset = tools_rig_utils.get_proxy_offset(scapula_node)
        paw_offset = tools_rig_utils.get_proxy_offset(paw_node)
        cmds.xform(scapula_offset, ws=True, a=True, ro=(0, 0, 0))
        ...
        self.scapula_dir_curve, scapula_dir_curve_offset = self.create_direction_curve(
            self.scapula_proxy,
            scapula_node,
        )
        core_attr.hide_lock_default_attrs(scapula_dir_curve_offset, scale=True, rotate=True, translate=True)
        ...
        self.paw_dir_curve, paw_dir_curve_offset = self.create_direction_curve(
            self.paw_proxy,
            paw_node,
        )
        ...
        side_x_offset = 90 * self.orientation.get_aim_axis()[0]
        self.setup_direction_curve(self.scapula_dir_curve, upperleg_node, up_vec=(1, 0, 0), x_offset=180)
        ...
        self.setup_direction_curve(self.paw_dir_curve, toes_node, x_offset=-side_x_offset)
        ...
        # Build solver to ensure co-plane relation between scapula, upperLeg and paw
        scapula_scp_joints = self.create_scp_joints()
        coplane_joints = self.create_coplane_joints()
        pole_joints = self.create_pole_joints()
        self.setup_proxy_coplane(scapula_scp_joints, coplane_joints, pole_joints)
        cmds.delete(cmds.listRelatives(self.scapula_dir_curve, children=True, type="aimConstraint")[0])
        ...
        cmds.orientConstraint(scapula_scp_joints[0], self.scapula_dir_curve, mo=False)
        ...
        _only_translation_proxies = [scapula_node, upperleg_node, lowerleg_node]
    ...
    def build_skeleton_hierarchy(self):
        ...
        """
        # Get joints
        scapula_jnt = tools_rig_utils.find_joint_from_uuid(self.scapula_proxy.get_uuid())
        ...
        for proxy in self.proxies:
            ...
            if proxy == self.scapula_proxy:
                proxy_obj_path = self.scapula_dir_curve
            elif proxy == self.upperleg_proxy:
        ...
        # Parent joints
        parent_uuid = self.scapula_proxy.get_parent_uuid()
        if parent_uuid:
            parent_joint_node = tools_rig_utils.find_joint_from_uuid(parent_uuid)
            core_hrchy.parent(source_objects=scapula_jnt, target_parent=parent_joint_node)
        core_hrchy.parent(source_objects=upperleg_jnt, target_parent=scapula_jnt)
    ...
    def build_rig(self, project_prefix=None, **kwargs):
        ...
        ball_jnt = tools_rig_utils.find_joint_from_uuid(self.ball_proxy.get_uuid())
        toesend_jnt = tools_rig_utils.find_joint_from_uuid(self.toesend_proxy.get_uuid())
        module_jnt_list = [scapula_jnt, upperleg_jnt, lowerleg_jnt, paw_jnt, toes_jnt, ball_jnt, toesend_jnt]
        ...
        fk_joints = self.create_knt_skeleton(module_jnt_list, joint_automation_grp, "fk")
        scapula_fk_jnt = fk_joints[0]
        upperleg_fk_jnt = fk_joints[1]
        lowerleg_fk_jnt = fk_joints[2]
        paw_fk_jnt = fk_joints[3]
        toes_fk_jnt = fk_joints[4]
        ball_fk_jnt = fk_joints[5]
        toesend_fk_jnt = fk_joints[6]

        ik_joints = self.create_knt_skeleton(module_jnt_list, joint_automation_grp, "ik")
        scapula_ik_jnt = ik_joints[0]
        upperleg_ik_jnt = ik_joints[1]
        lowerleg_ik_jnt = ik_joints[2]
        paw_ik_jnt = ik_joints[3]
        toes_ik_jnt = ik_joints[4]
        ball_ik_jnt = ik_joints[5]
        toesend_ik_jnt = ik_joints[6]
        ...
        # Scapula FK Control
        auto_pos_scapula_grp_suffix = "autoPos"
        scapula_fk_ctrl, scapula_fk_groups = self.create_rig_control(
            control_base_name=self.scapula_proxy.get_name(),
            curve_file_name="_circle_pos_x",
            parent_obj=global_offset_ctrl,
            extra_parent_groups=[auto_pos_scapula_grp_suffix],
            match_obj=scapula_jnt,
            add_offset_ctrl=False,
            rot_order=cmds.getAttr(f"{scapula_jnt}.rotateOrder"),
            shape_scale=quad_leg_scale * 0.3,
            color=core_color.get_directional_color(object_name=scapula_jnt),
        )[:2]
        self._add_driver_uuid_attr(
            target_driver=scapula_fk_ctrl,
            driver_type=tools_rig_const.RiggerDriverTypes.FK,
            proxy_purpose=self.scapula_proxy,
        )
        cmds.parentConstraint(scapula_fk_ctrl, scapula_fk_jnt, mo=True)
        cmds.parentConstraint(scapula_fk_ctrl, scapula_ik_jnt, mo=True)

        # UpperLeg FK Control
        upperleg_fk_ctrl, upperleg_fk_groups = self.create_rig_control(
            control_base_name=self.upperleg_proxy.get_name(),
            curve_file_name="_circle_pos_x",
            parent_obj=scapula_fk_ctrl,
        ...
        paw_ik_ctrl, paw_ik_groups = self.create_rig_control(
            ...
            shape_pos_offset=(0, _paw_toes_distance * 2, 0),
            shape_rot_offset=(40, 90, 180 * (not _is_right_side)),
            color=core_color.get_directional_color(object_name=paw_jnt),
        )[:2]
        self._add_driver_uuid_attr(
        ...
        # Scapula Pos Automation
        cmds.addAttr(scapula_fk_ctrl, ln="autoPosValue", nn="Auto Pos Value", at="float", keyable=True, min=0, max=1.0)
        cmds.setAttr(f"{scapula_fk_ctrl}.autoPosValue", 0.1)

        auto_pos_mult_name = f"{self._assemble_node_name(self.scapula_proxy.get_name())}_autoPos_mult"
        auto_pos_mult = cmds.createNode("multiplyDivide", n=auto_pos_mult_name)
        cmds.connectAttr(f"{toes_ik_ctrl}.ty", f"{auto_pos_mult}.input1X")
        cmds.connectAttr(f"{toes_ik_ctrl}.tz", f"{auto_pos_mult}.input1Y")
        cmds.connectAttr(f"{scapula_fk_ctrl}.autoPosValue", f"{auto_pos_mult}.input2X")
        cmds.connectAttr(f"{scapula_fk_ctrl}.autoPosValue", f"{auto_pos_mult}.input2Y")

        auto_pos_switch_name = f"{self._assemble_node_name(self.scapula_proxy.get_name())}_autoPos_switch"
        auto_pos_switch = cmds.createNode("multiplyDivide", n=auto_pos_switch_name)
        cmds.connectAttr(f"{auto_pos_mult}.outputX", f"{auto_pos_switch}.input1X")
        cmds.connectAttr(f"{auto_pos_mult}.outputY", f"{auto_pos_switch}.input1Y")
        cmds.connectAttr(f"{ik_switch_ctrl}.influenceSwitch", f"{auto_pos_switch}.input2X")
        cmds.connectAttr(f"{ik_switch_ctrl}.influenceSwitch", f"{auto_pos_switch}.input2Y")

        if _is_right_side:
            cmds.connectAttr(f"{auto_pos_switch}.outputX", f"{scapula_fk_groups[1]}.tx")
            auto_pos_rev_y_name = f"{self._assemble_node_name(self.scapula_proxy.get_name())}_autoPos_revY"
            auto_pos_rev_y = cmds.createNode("multiplyDivide", name=auto_pos_rev_y_name)
            cmds.setAttr(f"{auto_pos_rev_y}.input2X", -1)
            cmds.connectAttr(f"{auto_pos_switch}.outputY", f"{auto_pos_rev_y}.input1X")
            cmds.connectAttr(f"{auto_pos_rev_y}.outputX", f"{scapula_fk_groups[1]}.ty")
        else:
            cmds.connectAttr(f"{auto_pos_switch}.outputY", f"{scapula_fk_groups[1]}.ty")
            auto_pos_rev_z_name = f"{self._assemble_node_name(self.scapula_proxy.get_name())}_autoPos_revZ"
            auto_pos_rev_z = cmds.createNode("multiplyDivide", name=auto_pos_rev_z_name)
            cmds.setAttr(f"{auto_pos_rev_z}.input2X", -1)
            cmds.connectAttr(f"{auto_pos_switch}.outputX", f"{auto_pos_rev_z}.input1X")
            cmds.connectAttr(f"{auto_pos_rev_z}.outputX", f"{scapula_fk_groups[1]}.tx")
        ...
        core_attr.hide_lock_default_attrs([scapula_fk_ctrl], scale=True, visibility=True)
        ...
        for ctrls in [scapula_fk_ctrl, upperleg_fk_ctrl, lowerleg_ik_ctrl, toes_ik_ctrl]:
            core_attr.add_separator_attr(target_object=ctrls, attr_name=core_rigging.RiggingConstants.SEPARATOR_SPACE)
        if module_parent:
            _parent_joint = tools_rig_utils.find_joint_from_uuid(self.get_parent_uuid())
            tools_rig_utils.create_follow_enum_setup(
                control=scapula_fk_ctrl, parent_list=[_parent_joint, global_offset_ctrl], constraint_type="orient"
            )
            tools_rig_utils.create_follow_enum_setup(
                control=lowerleg_pv_parent,
                attribute_item=lowerleg_ik_ctrl,
                parent_list=[_parent_joint, auto_pole_joint_node, toes_ik_ctrl, global_offset_ctrl],
                default_value=2,
            )
            tools_rig_utils.create_follow_enum_setup(
                control=toes_ik_ctrl, parent_list=[_parent_joint, global_offset_ctrl], default_value=0
            )
        else:
            tools_rig_utils.create_follow_enum_setup(
                control=scapula_fk_ctrl, parent_list=[global_offset_ctrl], constraint_type="orient"
            )
            tools_rig_utils.create_follow_enum_setup(
                control=lowerleg_pv_parent,
                attribute_item=lowerleg_ik_ctrl,
            ...
            tools_rig_utils.create_follow_enum_setup(
                control=toes_ik_ctrl, parent_list=[global_offset_ctrl], default_value=0
            )
        # Upperleg space
        tools_rig_utils.create_follow_enum_setup(
            control=upperleg_fk_ctrl, parent_list=[scapula_fk_ctrl, global_offset_ctrl], constraint_type="orient"
        )
        ...
        self.module_children_drivers = [scapula_fk_groups[0]]
...
class ModuleQuadFrontLegLeft(ModuleQuadFrontLeg):
    def __init__(self, name="L Quad Front Leg", prefix=core_naming.NamingConstants.Prefix.LEFT, suffix=None):
        """
        Initializes the front left leg module for a quadruped character rig.

        Args:
            name (str): The display name of the module in the rigging UI. Defaults to "L Quad Front Leg".
            prefix (str, optional): A prefix to apply to all generated object names. Defaults to None.
            suffix (str, optional): A suffix to apply to all generated object names. Defaults to None.
        """
        super().__init__(name=name, prefix=prefix, suffix=suffix)
...
class ModuleQuadFrontLegRight(ModuleQuadFrontLeg):
    def __init__(self, name="R Quad Front Leg", prefix=core_naming.NamingConstants.Prefix.RIGHT, suffix=None):
        """
        Initializes the front right leg module for a quadruped character rig.

        Args:
            name (str): The display name of the module in the rigging UI. Defaults to "R Quad Front Leg".
            prefix (str, optional): A prefix to apply to all generated object names. Defaults to None.
            suffix (str, optional): A suffix to apply to all generated object names. Defaults to None.
        """
        super().__init__(name=name, prefix=prefix, suffix=suffix)
...
if __name__ == "__main__":
    ...
    # import gt.tools.auto_rigger.modules.module_quad_front_leg as tools_rig_mod_quad_front_leg
    import importlib

    importlib.reload(tools_rig_mod_root)
    importlib.reload(tools_rig_frm)
    importlib.reload(tools_rig_utils)
    # importlib.reload(tools_rig_mod_quad_front_leg)

    # Quadruped Front Leg Test
    a_root = tools_rig_mod_root.ModuleRoot()
    a_quad_front_left = ModuleQuadFrontLegLeft()
    a_quad_front_right = ModuleQuadFrontLegRight()

    root_uuid = a_root.root_proxy.get_uuid()
    a_quad_front_left.set_parent_uuid(root_uuid)
    a_quad_front_right.set_parent_uuid(root_uuid)

    a_project = tools_rig_frm.RigProject()
    a_project.add_to_modules(a_root)
    a_project.add_to_modules(a_quad_front_left)
    a_project.add_to_modules(a_quad_front_right)
    ...
    a_project.set_preference_value_using_key(key="hide_skeleton", value=False)
    ...
    # Show Orientation
    try:
        cmds.select(["L_frontScapula_JNT", "R_frontScapula_JNT"], hierarchy=True)
