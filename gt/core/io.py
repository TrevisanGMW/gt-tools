...
class DataDirConstants:
    ...
    DIR_UTILS = os.path.dirname(__file__).replace(os.sep, "/")  # forward slashes to prevent unicode error
...
def is_json_serializable(data, allow_none=True):
    """

    Args:
        data (any): The data to be checked. Serializable types are:
            None
            bool (i.e., True or False)
            int (and other integers like long in Python 2)
            float
            str (and unicode in Python 2)
            list, tuple
            dict (with string keys)
        allow_none (bool, optional): If True, it will return False when checking if "None" is serializable.

    Returns:
        bool: True if the data type is serializable, or False if not.
    """
