...
def get_uv_shells(obj):
    ...
    selection.add(obj)
    sel_iter = om.MItSelectionList(selection, om.MFn.kMesh)
    ...
    for uvset in uv_sets:
        shape_fn = om.MFnMesh(shape_path)
        shells = om.MScriptUtil()
        shells.createFromInt(0)
        nb_uv_shells = shells.asUintPtr()

        u_array = om.MFloatArray() # U coords
        v_array = om.MFloatArray() # V coords
        shell_ids = om.MIntArray() #The container for the uv shell Ids
 
        shape_fn.getUVs(u_array, v_array)
        shape_fn.getUvShellsIds(shell_ids, nb_uv_shells, uvset)
        ...
        all_sets.append({uvset: shells})
    return all_sets

def are_uvs_identical(obj_a, obj_b):
    '''
    Compares the UVs of objects A and B and returns whether or not they are different.
