...
# Logging Setup
logging.basicConfig()
logger = logging.getLogger("gt_create_testing_keys")
...
def build_gui_create_testing_keys():
    ...
    window_gui_create_testing_keys = cmds.window(window_name, title=script_name + '  (v' + script_version + ')',
    ...
    cmds.rowColumnLayout(nc=4, cw=[(1, 20), (2, 63), (3, 63), (4, 63)], cs=[(1, 6), (2, 0), (3, 2), (4, 2)],
                         p=body_column)
    ...
    interval_text_field = cmds.floatSliderGrp(cw=[(1, 133), (2, 45), (3, 0)], cal=[(1, 'left')], field=True,
                                              label='Interval Between Frames:',
                                              minValue=0, maxValue=1000, fieldMinValue=0, fieldMaxValue=1000,
                                              value=5)  # , cc=lambda args: update_grade_output())
    ...
    def validate_operation():
        """
        Validates operation before running (Undo chunk, selection, etc)
        """
        errors = ''
        try:
        ...
        if errors != '':
            cmds.warning('An error occurred when creating the keyframes. Open the script editor for more information.')
            print('######## Errors: ########')
            print(errors)
            print('#########################')
    ...
    def create_testing_keyframes(offset, attr, interval, create_inverted=False, ws_method=True):
        ...
        Args:
            ...
            create_inverted (bool): Whether to increase a key with the inverted offset value
            ws_method (bool): Whether to use world-space or object-space
        """

        selection = cmds.ls(selection=True)
        # Gets the first frame available in the timeline (usually 0 or 1)
        current_frame = cmds.playbackOptions(animationStartTime=True, q=True)
        # Gets the max time (work area end frame) to see if it needs to expand the timeline
        current_max_time = cmds.playbackOptions(maxTime=True, q=True)
    ...
    cmds.showWindow(window_gui_create_testing_keys)
...
def build_gui_help_create_testing_keys():
    ...
    cmds.text(l='These are the values that will be added to the object.\nIf set to "0.0" it will be ignored. '
                '(No keys will be created)', align="left")
    cmds.separator(h=15, style='none')  # Empty Space
    ...
    cmds.text(l="Deletes all keys attached to the selected controls before\ncreating new ones. "
                "(Doesn't include Set Driven Keys)", align="left")
    cmds.separator(h=15, style='none')  # Empty Space
    cmds.text(l='Use World Space (WS) Values:', align="left", fn="boldLabelFont")
    cmds.text(
        l='Moves objects as if they were not part of a hierarchy,\nwhich mimics the behaviour of the world space.',
        align="left")
    cmds.separator(h=15, style='none')  # Empty Space
