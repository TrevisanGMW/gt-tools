...
def build_gui_make_ik_stretchy():
    ...
    # Build UI
    build_gui_make_ik_stretchy = cmds.window(window_name, title=script_name + '  (v' + script_version + ')',\
    ...
    ik_handle_container = cmds.rowColumnLayout(nc=2, cw=[(1, 129),(2, 130)], cs=[(1,0)])
    ik_handle_btn = cmds.button(l ="Load IK Handle", c=lambda x:object_load_handler("ik_handle"), w=130)
    ...
    attr_holder_container = cmds.rowColumnLayout(nc=2, cw=[(1, 129),(2, 130)], cs=[(1,0)], p=body_column)
    attr_holder_btn = cmds.button(l ="Load Attribute Holder", c=lambda x:object_load_handler("attr_holder"), w=130)
    ...
    cmds.showWindow(build_gui_make_ik_stretchy)
...
def build_gui_help_make_stretchy_ik():
    ...
    cmds.text(l='Use this button to load your attribute holder.\nThis is usually a control.\nA few custom attributes will be added to this object,\n so the user can control the stretchy system.', align="center")
    
    cmds.separator(h=15, style='none') # Empty Space
    cmds.text(l='Important:', align="center", fn="boldLabelFont")
    cmds.text(l='The ikHandle cannot be outside of a group.\n So it will be automatically grouped when this is the case.', align="center")
    cmds.separator(h=5, style='none') # Empty Space
    cmds.text(l='If an attribute holder is not provided, a simpler version\n of the stretchy system will be created instead.', align="center")
    cmds.separator(h=15, style='none') # Empty Space
...
def select_existing_object(obj):
    ...
    if obj != '':
        if cmds.objExists(obj):
            ...
            cmds.inViewMessage(amg=unique_message + '<span style=\"color:#FF0000;text-decoration:underline;\">'+ str(obj) +'</span><span style=\"color:#FFFFFF;\"> selected.</span>', pos='botLeft', fade=True, alpha=.9)
        else:
            cmds.warning('"' + str(obj) + '" couldn\'t be selected. Make sure you didn\'t rename or deleted the object after loading it')
    else:
