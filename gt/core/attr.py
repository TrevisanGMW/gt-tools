...
# --------------------------------------------- Getters ---------------------------------------------
def get_attr(
    attribute_path=None, obj_name=None, attr_name=None, enum_as_string=False, verbose=True, log_level=logging.INFO
):
    """
    ...
    if attribute_path and (obj_name or attr_name):
        message = (
            f"Unable to get attribute value. Multiple get methods were provided in the same function. "
...
def get_multiple_attr(
    ...
    if attribute_path and isinstance(attribute_path, str):
        try:
            value = get_attr(
                attribute_path=attribute_path, enum_as_string=enum_as_string, verbose=verbose, log_level=log_level
            )
    ...
    if obj_list and attr_list and isinstance(obj_list, list) and isinstance(attr_list, list):
        for attr in attr_list:
            for obj in obj_list:
                ...
                try:
                    value = get_attr(
                        attribute_path=attr_path, enum_as_string=enum_as_string, verbose=verbose, log_level=log_level
                    )
...
def get_trs_attr_as_formatted_string(
    obj_list, decimal_place=2, add_description=False, add_object=True, separate_channels=False, strip_zeroes=True
):
    """
    ...
    _new_line = "\n"
    output = core_str.remove_prefix(output, _new_line)
    output = core_str.remove_suffix(output, _new_line)
    return output
...
def selection_delete_user_defined_attrs(delete_locked=True, feedback=True):
    ...
    try:
        deleted_attrs = delete_user_defined_attrs(obj_list=selection, delete_locked=delete_locked, verbose=True) or []
        deleted_counter = 0
        if deleted_attrs:
...
def copy_attr(source_attr_path, target_list, prefix=None, override_name=None, override_keyable=None):
    """
    ...
    Args:
        ...
        override_name (str, optional): If provided, it overrides the name of the copied attribute.
        override_keyable (bool, optional): If provided it will overwrite the keyable state of the copied attribute.
    ...
    if isinstance(override_keyable, bool):
        attr_data["is_keyable"] = override_keyable
    ...
    if override_name and isinstance(override_name, str):
        target_attr_name = override_name
    else:
        target_attr_name = source_attr

    # Adjust attribute name with prefix if provided
    if prefix:
        target_attr_name = f"{prefix}{core_str.upper_first_char(target_attr_name)}"
...
# -------------------------------------------- Connection -------------------------------------------
def connect_attr(
    source_attr, target_attr_list, force=False, verbose=False, log_level=logging.INFO, raise_exceptions=False
):
    """
    This function sets locked or hidden states of specified attributes of objects using Maya's `cmds.setAttr` function.
    It provides options to set locked or hidden states for a single attribute path, multiple objects and attributes.
